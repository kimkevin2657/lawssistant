...
<div class="vi_info">
	...
	<div class="vi_txt_bx">
        ...
        <?php // 추춴ID, 후원ID 입력 ; ?>
        <?php if( $gs['buy_partner_grade'] && is_partner($member['id']) ) : ?>
            <?php
            $myGrade  = Partner::findTopId($member['id']);

            if( isset($myGrade[$gs['buy_partner_grade']]) ) {
                $my_top_id = $myGrade[$gs['buy_partner_grade']];
            } else {
                $my_top_id = 'admin';
            }
            $io_pt_id = $my_top_id;
            $io_up_id = $my_top_id;
            ?>
        <input type="hidden" id="buy_partner_grade" name="buy_partner_grade" value="<?php echo $gs['buy_partner_grade']; ?>">
        <input type="hidden" id="current_grade" name="current_grade" value="<?php echo $member['grade']; ?>">
            <div id="option_set_upper">
                <ul id="option_set_upper_added">
                    <li class="sit_opt_list vi_txt_li<?php echo false && $gs['buy_partner_grade'] == $member['grade'] && is_partner($member['id']) ? ' dpn ' : ''; ?>">
                        <dl>
                            <dt>
                                <span class="sit_opt_subj"><label for="io_up_id">후원ID</label></span>
                                <span class="sit_opt_prc"></span>
                            </dt>
                            <dd class="li_ea">
                    ...
                    <li class="sit_opt_list vi_txt_li">
                        ...
                        <dl>
                            <dt>
                                <span class="sit_opt_subj"><label for="io_pt_id">추천ID</label></span>
                                <span class="sit_opt_prc"></span>
                            </dt>
                            <dd class="li_ea">
                ...
                (function($){
                    $(document).ready(function(){
                        $('#io_up_id, #io_pt_id').on('blur', function(){
                            var grade = $('#buy_partner_grade').val();
                            var id    = $(this).val();
                            if( id == '' || id == 'admin' || id == 'k2k9' ) return;
                            $.ajax({
                                url : '/plugin/zentool/partner/ajax.find_up_id.php',
		...
		<?php if(!$is_pr_msg) { ?>
		<div class="vi_btn">
			<?php echo get_buy_button($script_msg, $index_no, $gs['buy_partner_grade'] ? 1 : 3); ?>
		</div>
...
<section class="mart50">
	...
	<div class="mart15">
		<?php
		Theme::get_theme_part(TB_THEME_PATH,'/view_user.skin.php');
		?>
	</div>
</section>
...
<section class="mart50">
	...
	<div class="mart15 vi_qa">
		<?php
		Theme::get_theme_part(TB_THEME_PATH,'/view_qa.skin.php');
		?>
	</div>
</section>
