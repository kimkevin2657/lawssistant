...
if($type == 1)      $tb['title'] = $gw_dp_label['q_type1'];//'쇼핑특가';
else if($type == 2) $tb['title'] = $gw_dp_label['q_type2'];//'베스트셀러';
else if($type == 3) $tb['title'] = $gw_dp_label['q_type3'];//'신상품';
else if($type == 4) $tb['title'] = $gw_dp_label['q_type4'];//'인기상품';
else if($type == 5) $tb['title'] = $gw_dp_label['q_type5'];//'추천상품';
else
...
Theme::get_theme_part(TB_MTHEME_PATH,'/listtype.skin.php');
