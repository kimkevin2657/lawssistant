...
if ( $req_tx == "pay" )
{
    if( $res_cd == "0000" )
    {
        ...
        $pnt_issue = $c_PayPlus->mf_get_res_data( "pnt_issue" ); // 결제 쇼핑포인트사 코드
...
/* = -------------------------------------------------------------------------- = */
        if ( $use_pay_method == "100000000000" )
        {
            ...
            /* = -------------------------------------------------------------- = */
            /* =   05-1.1. 복합결제(쇼핑포인트+신용카드) 승인 결과 처리               = */
            /* = -------------------------------------------------------------- = */
            if ( $pnt_issue == "SCSK" || $pnt_issue == "SCWB" )
            {
                ...
                $add_pnt      = $c_PayPlus->mf_get_res_data ( "add_pnt"      ); // 발생 쇼핑포인트
                $use_pnt      = $c_PayPlus->mf_get_res_data ( "use_pnt"      ); // 사용가능 쇼핑포인트
                $rsv_pnt      = $c_PayPlus->mf_get_res_data ( "rsv_pnt"      ); // 총 누적 쇼핑포인트
...
/* = -------------------------------------------------------------------------- = */
        if ( $use_pay_method == "000100000000" )
        {
            ...
            $add_pnt      = $c_PayPlus->mf_get_res_data( "add_pnt"      ); // 발생 쇼핑포인트
            $use_pnt      = $c_PayPlus->mf_get_res_data( "use_pnt"      ); // 사용가능 쇼핑포인트
            $rsv_pnt      = $c_PayPlus->mf_get_res_data( "rsv_pnt"      ); // 적립 쇼핑포인트
        }
