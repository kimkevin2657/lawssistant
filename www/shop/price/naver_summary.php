...
$sql_search = " and SUBSTRING(a.update_time, 1, 10) = '".MS_TIME_YMD."' ";
...
for($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	$item_link = MS_SHOP_URL.'/view.php?index_no='.$row['index_no'];
    
    // 상태
    $class = 'U';
    $stock_qty = get_it_stock_qty($row['index_no']);

    if(substr($row['reg_time'], 0, 10) == MS_TIME_YMD && $row['update_time'] >= $row['reg_time'])
        $class = 'I';
