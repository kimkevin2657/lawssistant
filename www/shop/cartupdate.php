...
if($act == "buy")
{
   ...
   if($is_member) // 회원인 경우
        goto_url(MS_SHOP_URL."/orderform.php");
    else
		goto_url(MS_BBS_URL.'/login.php?url='.urlencode('/shop/orderform.php'));
}
else if($act == "alldelete") // 모두 삭제이면
{
	$sql = "select * from shop_cart where ct_select='0' and ct_direct='{$member['id']}'";
	$res = sql_query($sql);
	while($row=sql_fetch_array($res)) {
			 $gradeInfo = sql_fetch("select * from shop_order
					  where od_id = '{$row['od_id']}'
    ...
    $sql = " delete from shop_cart where ct_select='0' and ct_direct='{$member['id']}' ";
    sql_query($sql);
}
else if($act == "seldelete") // 선택삭제
{
    ...
    for($i=0; $i<$fldcnt; $i++) {
        ...
        if($ct_chk) {
            ...
            $sql = " delete from shop_cart where gs_id='$gs_id' and ct_select='0' and ct_direct='{$member['id']}' ";
            sql_query($sql);
        }
    }
}
