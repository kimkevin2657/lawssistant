...
if(MS_IS_MOBILE) {
	goto_url(MS_MSHOP_URL.'/list.php?ca_id='.$ca_id);
}
...
if( $total_count == 1 && $row['min_no'] == $row['max_no'] ) {
   ...
   // goto_url(MS_SHOP_URL.'/view.php?index_no='.$row['min_no']);
}
...
$sql = " select a.*, c.best, c.catecode $sql_common $sql_order limit $from_record, $rows ";
//echo $sql;
...
Theme::get_theme_part(MS_THEME_PATH,'/list.skin.php');
