...
$INIpayHome = MS_SHOP_PATH.'/inicis'; // 이니페이 홈디렉터리
...
if($_REQUEST){
$cc = print_r($_REQUEST,true);
$sql = "insert into ini_log_test(log,ip,ok) values('$cc','$PG_IP','T')";
sql_query($sql, FALSE);	
}
...
if( $PG_IP == "203.238.37.3" || $PG_IP == "203.238.37.15" || $PG_IP == "203.238.37.16" || $PG_IP == "203.238.37.25" || $PG_IP == "39.115.212.9" )  //PG에서 보냈는지 IP로 체크
{
		...
		$msg_id = $_REQUEST['msg_id'];             //메세지 타입
        $no_tid = $_REQUEST['no_tid'];             //거래번호
        $no_oid = $_REQUEST['no_oid'];             //상점 주문번호
        $id_merchant = $_REQUEST['id_merchant'];   //상점 아이디
        $cd_bank = $_REQUEST['cd_bank'];           //거래 발생 기관 코드
        $cd_deal = $_REQUEST['cd_deal'];           //취급 기관 코드
        $dt_trans = $_REQUEST['dt_trans'];         //거래 일자
        $tm_trans = $_REQUEST['tm_trans'];         //거래 시간
        $no_msgseq = $_REQUEST['no_msgseq'];       //전문 일련 번호
        $cd_joinorg = $_REQUEST['cd_joinorg'];     //제휴 기관 코드


        $dt_transbase = $_REQUEST['dt_transbase']; //거래 기준 일자
        $no_transeq = $_REQUEST['no_transeq'];     //거래 일련 번호
        $type_msg = $_REQUEST['type_msg'];         //거래 구분 코드
        $cl_close = $_REQUEST['cl_close'];         //마감 구분코드
        $cl_kor = $_REQUEST['cl_kor'];             //한글 구분 코드
        $no_msgmanage = $_REQUEST['no_msgmanage']; //전문 관리 번호
        $no_vacct = $_REQUEST['no_vacct'];         //가상계좌번호
        $amt_input = $_REQUEST['amt_input'];       //입금금액
        $amt_check = $_REQUEST['amt_check'];       //미결제 타점권 금액
        $nm_inputbank = $_REQUEST['nm_inputbank']; //입금 금융기관명
        $nm_input = $_REQUEST['nm_input'];         //입금 의뢰인
        $dt_inputstd = $_REQUEST['dt_inputstd'];   //입금 기준 일자
        $dt_calculstd = $_REQUEST['dt_calculstd']; //정산 기준 일자
        $flg_close = $_REQUEST['flg_close'];       //마감 전화
		...
		$sql = " update shop_order
					set receipt_time = '".MS_TIME_YMDHIS."'
					  , dan = '2'
				  where od_id = '$no_oid'
	...
	if($no_oid){
		$cc = print_r($_REQUEST,true);
		$sql = "insert into ini_log_test(log,ip,ok) values('$cc','$PG_IP','ok')";
		sql_query($sql, FALSE);	
	}
