...
include_once(MS_LIB_PATH.'/naverpay.lib.php');
...
define('NAVERPAY_BACK_URL', MS_SHOP_URL.$back_uri);
...
for($i=0; $i<$count; $i++) {
    ...
    //--------------------------------------------------------
    for($k=0; $k<$opt_count; $k++) {
        $io_id = preg_replace(MS_OPTION_ID_FILTER, '', trim(stripslashes($_POST['io_id'][$gs_id][$k])));
