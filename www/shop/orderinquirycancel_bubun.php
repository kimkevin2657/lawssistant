...
if($od['dan'] == '2'){
	if($od['use_price'] == '0'){
		...
		//$od['use_price'] = $od['use_price'] - $od['salebaesong_price'];
...
function form_check(f)
{
    ...
    if(!tax_mny && !free_mny) {
        alert("과세 취소금액 또는 비과세 취소금액을 입력해 주십시오.",MS_SHOP_URL."/orderinquiryview.php?od_id=$od_id");
        return false;
    }

    if((tax_mny && free_mny) && (tax_mny + free_mny) > max_mny) {
        alert("과세, 비과세 취소금액의 합을 "+number_format(String(max_mny))+"원 이하로 입력해 주십시오.",MS_SHOP_URL."/orderinquiryview.php?od_id=$od_id");
        return false;
    }

    if(tax_mny && tax_mny > max_mny) {
        alert("과세 취소금액을 "+number_format(String(max_mny))+"원 이하로 입력해 주십시오.",MS_SHOP_URL."/orderinquiryview.php?od_id=$od_id");
        return false;
    }

    if(free_mny && free_mny > max_mny) {
        alert("비과세 취소금액을 "+number_format(String(max_mny))+"원 이하로 입력해 주십시오.",MS_SHOP_URL."/orderinquiryview.php?od_id=$od_id");
        return false;
    }
