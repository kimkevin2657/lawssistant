            ...
            for ($day = 1; $day <= $total_day; $day++) {
                ...
                if ($cnt_time > 0 && !$is_block && $rm['rm_week'.$weekno]) {
                    ...
                    foreach ($arr_time as $k => $v) {
                        ...
                        $rm_html .= '<li class="'.$closed_tag.'"><a href="./wzb_booking_list.php?code=wzb_booking_list&sch_cp_ix='.$sch_cp_ix.'&sch_room='.urlencode($rm['rm_subject']).'&sch_frdate1='.$vdate.'&sch_todate1='.$vdate.'">'.$rmt_time.' ('.$rms_cnt .'/'. $rm_cnts.')</a></li>'.PHP_EOL;
                    }
                    $rm_html .= '  </ul>'.PHP_EOL;
                }
