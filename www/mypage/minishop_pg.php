...
<form name="fregform" id="fregform" action="./minishop_pg_update.php" method="post">
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td class="td_label">
			<input type="checkbox" name="de_bank_use" value="1" id="de_bank_use"<?php echo get_checked($minishop['de_bank_use'], "1"); ?>>
			<label for="de_bank_use">무통장</label>
			<input type="checkbox" name="de_card_use" value="1" id="de_card_use"<?php echo get_checked($minishop['de_card_use'], "1"); ?>>
			<label for="de_card_use">신용카드</label>
			<input type="checkbox" name="de_iche_use" value="1" id="de_iche_use"<?php echo get_checked($minishop['de_iche_use'], "1"); ?>>
			<label for="de_iche_use">실시간계좌이체</label>
			<input type="checkbox" name="de_vbank_use" value="1" id="de_vbank_use"<?php echo get_checked($minishop['de_vbank_use'], "1"); ?>>
			<label for="de_vbank_use">가상계좌</label>
			<input type="checkbox" name="de_hp_use" value="1" id="de_hp_use"<?php echo get_checked($minishop['de_hp_use'], "1"); ?>>
	...
	<tr>
		...
		<td> 
			<select name="de_pg_service" id="de_pg_service">
				<?php echo option_selected("kcp", $minishop['de_pg_service'], "NHN KCP"); ?>
				<?php echo option_selected("lg",  $minishop['de_pg_service'], "LG유플러스"); ?>	
				<?php echo option_selected("inicis", $minishop['de_pg_service'], "KG이니시스"); ?>
			</select> 
	...
	<tr>
		...
		<td>
			...
			<?php echo get_checked($minishop['de_tax_flag_use'], "1"); ?>>
	...
	<tr>
		...
		<td>
			...
			<?php echo get_checked($minishop['de_taxsave_use'], "1"); ?>>
	...
	<tr>
		...
		<td>
			...
			<?php echo get_checked($minishop['de_card_noint_use'], "1"); ?>>
	...
	<tr>
		...
		<td>
			...
			<?php echo get_checked($minishop['de_easy_pay_use'], "1"); ?>>
...
<div class="pg_info_fld kcp_info_fld">
	...
	<div class="tbl_frm01">
		...
		<tr>
			...
			<td>
				<input type="text" name="de_kcp_mid" value="<?php echo $minishop['de_kcp_mid']; ?>" id="de_kcp_mid" class="frm_input" size="10">					
		...
		<tr>
			...
			<td>
				<input type="text" name="de_kcp_site_key" value="<?php echo $minishop['de_kcp_site_key']; ?>" id="de_kcp_site_key" class="frm_input" size="40">
		...
		<tr>
			...
			<td>
				<?php echo MS_SHOP_URL; ?>/settle_kcp_common.php
...
<div class="pg_info_fld lg_info_fld">
	...
	<div class="tbl_frm01">
		...
		<tr>
			...
			<td>
				<input type="text" name="de_lg_mid" value="<?php echo $minishop['de_lg_mid']; ?>" id="de_lg_mid" class="frm_input" size="10" maxlength="20">
		...
		<tr>
			...
			<td>
				<input type="text" name="de_lg_mert_key" value="<?php echo $minishop['de_lg_mert_key']; ?>" id="de_lg_mert_key" class="frm_input" size="40" maxlength="50">
...
<div class="pg_info_fld inicis_info_fld">
	...
	<div class="tbl_frm01">
		...
		<tr>
			...
			<td>
				<input type="text" name="de_inicis_mid" value="<?php echo $minishop['de_inicis_mid']; ?>" id="de_inicis_mid" class="frm_input" size="10">
		...
		<tr> 
			...
			<td>
				<input type="text" name="de_inicis_admin_key" value="<?php echo $minishop['de_inicis_admin_key']; ?>" id="de_inicis_admin_key" class="frm_input" size="5" maxlength="4">
		...
		<tr>
			...
			<td>					
				<input type="text" name="de_inicis_sign_key" value="<?php echo $minishop['de_inicis_sign_key']; ?>" id="de_inicis_sign_key" class="frm_input" size="40" maxlength="50">
		...
		<tr>
			...
			<td>					
				<input type="checkbox" name="de_samsung_pay_use" value="1" id="de_samsung_pay_use"<?php echo get_checked($minishop['de_samsung_pay_use'], "1"); ?>>
		...
		<tr>
			<th scope="row">가상계좌 입금통보 URL</th>
			<td>
				<?php echo MS_SHOP_URL; ?>/settle_inicis_common.php
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td> 
			<input type="radio" name="de_escrow_use" value="0" id="de_escrow_use_no"<?php echo get_checked($minishop['de_escrow_use'], "0"); ?>>
			<label for="de_escrow_use_no">일반결제 사용</label>
			<input type="radio" name="de_escrow_use" value="1" id="de_escrow_use_yes"<?php echo get_checked($minishop['de_escrow_use'], "1"); ?>>
...
<div class="tbl_frm01">
	...
	<tbody>
	<?php
	$bank = unserialize($minishop['de_bank_account']);
	for($i=0; $i<5; $i++) {
		$k = ($i+1);
	?>
	<tr>
...
$(function() { 
    $(".pg_info_fld").hide();
    <?php if($minishop['de_pg_service']) { ?>
    $(".<?php echo $minishop['de_pg_service']; ?>_info_fld").show();
    <?php } else { ?>
    $(".kcp_info_fld").show();
    <?php } ?>
...
<?php
// 결제모듈 실행권한 체크
if($minishop['de_iche_use'] || $minishop['de_vbank_use'] || $minishop['de_hp_use'] || $minishop['de_card_use']) {
    // kcp의 경우 pp_cli 체크
    if($minishop['de_pg_service'] == 'kcp') {
        ...
        echo module_exec_check(MS_SHOP_PATH.$exe, 'pp_cli');

        // shop/kcp/log 디렉토리 체크 후 있으면 경고
        if(is_dir(MS_SHOP_PATH.'/kcp/log') && is_writable(MS_SHOP_PATH.'/kcp/log')) {
            echo '<script>'.PHP_EOL;
    ...
    // LG의 경우 log 디렉토리 체크
    if($minishop['de_pg_service'] == 'lg') {
        $log_path = MS_LGXPAY_PATH.'/lgdacom/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(MS_PATH.'/', '', MS_LGXPAY_PATH).'/lgdacom 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
            if(!is_writable($log_path)) {
                echo '<script>'.PHP_EOL;
                echo 'alert("'.str_replace(MS_PATH.'/', '',$log_path).' 폴더에 쓰기권한을 부여해 주십시오.\n> chmod 707 log");'.PHP_EOL;
                echo '</script>'.PHP_EOL;
            }
        }
    }
    ...
    if($minishop['de_pg_service'] == 'inicis') {
        ...
        $log_path = MS_SHOP_PATH.'/inicis/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(MS_PATH.'/', '', MS_SHOP_PATH).'/inicis 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
            if(!is_writable($log_path)) {
                echo '<script>'.PHP_EOL;
