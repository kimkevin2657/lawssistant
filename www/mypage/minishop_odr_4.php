...
// 주문서 query 공통
include_once("./minishop_odr_query.php");
...
<a href="./minishop_odr_excel.php?$q1" class="btn_lsmall white"><i class="fa fa-file-excel-o"></i> 검색결과 엑셀저장</a>
EOF;
?>
...
<form name="forderlist" id="forderlist" action="./minishop_odr_update.php" onsubmit="return forderlist_submit(this);" method="post">
...
<div class="tbl_head01">
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td rowspan="<?php echo $rowspan; ?>">
			<a href="<?php echo MS_MYPAGE_URL; ?>/minishop_odr_form.php?od_id=<?php echo $row['od_id']; ?>" onclick="win_open(this,'minishop_odr_form','1200','800','yes');return false;" class="fc_197"><?php echo $row['od_id']; ?></a>
		...
		<td class="td_imgline"><a href="<?php echo MS_SHOP_URL; ?>/view.php?index_no=<?php echo $row2['gs_id']; ?>" target="_blank"><?php echo get_od_image($row['od_id'], $gs['simg1'], 30, 30); ?></a></td>
		<td class="td_itname"><a href="<?php echo MS_MYPAGE_URL; ?>/page.php?code=minishop_goods_form&w=u&gs_id=<?php echo $row2['gs_id']; ?>" target="_blank"><?php echo get_text($gs['gname']); ?></a></td>
		<td>
	...
	$("#frmOrderPrint, #frmOrderExcel").on("click", function() {
		...
		if(od_id == "") {
			...
			if(type == 'frmOrderPrint') {
				var url = "./minishop_odr_print.php?od_id="+od_id;
				window.open(url, "frmOrderPrint", "left=100, top=100, width=670, height=600, scrollbars=yes");
				return false;
			} else {
				this.href = "./minishop_odr_excel2.php?od_id="+od_id;
				return true;
			}
		}
	});
