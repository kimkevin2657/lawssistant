...
<form name="fgoodslist" id="fgoodslist" method="post" action="./minishop_goods_admlist_update.php" onsubmit="return fgoodslist_submit(this);">
...
<div class="tbl_head02">
	...
	<tr>
		...
		<th scope="col"><?php echo subject_sort_link('a.gname',$q2); ?>상품명</a></th>
	...
	<tr class="rows">
		...
		<th class="th_bg"><?php echo subject_sort_link('a.normal_price',$q2); ?>시중가</a></th>		
		<th class="th_bg"><?php echo subject_sort_link('a.goods_price',$q2); ?>판매가</a></th>
		<th class="th_bg"><?php echo subject_sort_link('a.gpoint',$q2); ?>쇼핑포인트</a></th>
		<?php if($config['pf_payment_yes']) { ?>
	...
	<tr class="<?php echo $bg; ?>">
		...
		<?php if($config['pf_payment_yes']) { 
		if($member['payflag'] == '1'){ //1 원
			$row['goods_kv'] = $row['goods_kv'] + $member['payment'];
		}else{ //0 per
			$row['goods_kv_per'] = $row['goods_kv_per'] + $member['payment'];
		}
		?>
		<td rowspan="2" class="tar fc_00f"><?php echo number_format($row['goods_kv']); ?>원<br><?php echo number_format($row['goods_kv_per']); ?>%</td>
		<?php } ?>
	</tr>
	<tr class="<?php echo $bg; ?>">
