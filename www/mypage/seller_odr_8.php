...
<div class="tbl_head01">
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td rowspan="<?php echo $rowspan; ?>">
			<a href="<?php echo MS_MYPAGE_URL; ?>/seller_odr_form.php?od_id=<?php echo $row['od_id']; ?>" onclick="win_open(this,'seller_odr_form','1200','800','yes');return false;" class="fc_197"><?php echo $row['od_id']; ?></a>
			<?php echo $sodr['disp_mobile']; ?>
			<?if($row['return_status']=="Y"){?>
				<br /><font color="red">반품 처리완료</font>
			<?}else{?>
				<br /><font color="blue">반품 처리중</font>
			<?}?>
		</td>
		<td rowspan="<?php echo $rowspan; ?>">
		...
		<td class="td_itname"><a href="<?php echo MS_MYPAGE_URL; ?>/page.php?code=seller_goods_form&w=u&gs_id=<?php echo $row2['gs_id']; ?>" target="_blank"><?php echo get_text($gs['gname']); ?></a></td>
		<?php if($k == 0) { ?>
		<td rowspan="<?php echo $rowspan; ?>">
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			<input type="submit" name="act_button" value="교환 처리완료" class="btn_medium red" onclick="document.pressed=this.value">
			<input type="submit" name="act_button" value="교환 처리중" class="btn_medium white" onclick="document.pressed=this.value">
			<input type="submit" name="act_button" value="교환 취소후 배송중으로 처리" class="btn_medium white" onclick="document.pressed=this.value">
		</td>
	</tr>
	</tbody>
	</table>
...
function forderlist_submit(f)
{
    ...
    if(!confirm("선택하신 주문서의 처리상태를 '"+document.pressed+"'로 변경하시겠습니까?"))
        return false;
