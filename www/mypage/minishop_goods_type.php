...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
            <?php
            $dpLabels = Shop::dpLabel($member['id'], array('use_yn'=>'Y'));
            foreach($dpLabels as $typeNo=>$dpLabel) :
                $valkey = 'q_type'.$typeNo;
            ?>
            <?php echo check_checked($valkey, ${$valkey}, '1', $dpLabel['type_label']); ?>
            <?php
            endforeach;
            ?>
		</td>
...
<form name="fgoodslist" method="post" action="./minishop_goods_type_update.php">
...
<div class="tbl_head01">
	...
	<colgroup>
		...
		<col class="w80">
		<col>
        <?php foreach($dpLabels as $typeNo=>$dpLabel ) : ?>
		<col class="w50">
        <?php endforeach; ?>
		<col class="w80">
		<col class="w80">
	...
	<tr>
		...
		<th scope="col">상품명</th>
        <?php foreach($dpLabels as $typeNo=>$dpLabel ) : ?>
            <th scope="col" class="th_bg lh2"><?php echo preg_replace('/(셀러|특가|상품|관)/', '<br>$1', $dpLabel['type_label']); //쇼핑<br>특가 ?></th>
        <?php endforeach; ?>
		<th scope="col">진열</th>
		<th scope="col">재고</th>
		<th scope="col">판매가</th>
	...
	<tr class="<?php echo $bg; ?>">
        ...
        <?php
        foreach($dpLabels as $typeNo=>$dpLabel ) :
            $row2key = 'it_type'.$typeNo;
            ?>
        <td><input type="checkbox" name="<?php echo $row2key; ?>[<?php echo $i; ?>]" value="1"<?php echo get_checked($row2[$row2key],"1"); ?>></td>
        <?php
        endforeach; ?>
