...
<form name="flogo" method="post" action="./minishop_logo_update.php" enctype="MULTIPART/FORM-DATA">
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			...
			<?php
			$file = MS_DATA_PATH.'/banner/'.$lg['basic_logo'];
			if(is_file($file) && $lg['basic_logo']) {
				$basic_logo = rpc($file, MS_PATH, MS_URL);
			?>
	...
	<tr>
		...
		<td>
			...
			<?php
			$file = MS_DATA_PATH.'/banner/'.$lg['mobile_logo'];
			if(is_file($file) && $lg['mobile_logo']) {
				$mobile_logo = rpc($file, MS_PATH, MS_URL);
			?>
	...
	<tr>
		...
		<td>
			...
			<?php
			$file = MS_DATA_PATH.'/banner/'.$lg['sns_logo'];
			if(is_file($file) && $lg['sns_logo']) {
				$sns_logo = rpc($file, MS_PATH, MS_URL);
			?>
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			...
			<?php
			$file = MS_DATA_PATH.'/banner/'.$lg['favicon_ico'];
			if(is_file($file) && $lg['favicon_ico']) {
				$favicon_ico = rpc($file, MS_PATH, MS_URL);
			?>			
	...
	<tr>
		<td>
			...
			<p class="padt5"><img src="<?php echo MS_IMG_URL; ?>/visual_favicon.jpg"></p>
		</td>
	</tr>
	</tbody>
	</table>
