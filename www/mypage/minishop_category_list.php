...
minishop::maybeCreateCategory($member['id']);
...
<h2>카테고리 등록</h2>
<form name="fcategoryform" method="post" action="./minishop_category_update.php" enctype="MULTIPART/FORM-DATA">
...
<div class="sho_cate_bx">
	<div class="local_frm02">
		<a href="./minishop_category_excel.php" class="btn_lsmall bx-white"><i class="fa fa-file-excel-o"></i> 카테고리 엑셀저장</a>
		<a href="./minishop_category_def.php?mod_type=R" class="btn_lsmall bx-red" onclick="return default_confirm(this, '');"><i class="fa fa-refresh fa-spin"></i> 본사와 동일하게 설정값 초기화</a>
	</div>
	<ul>
	<?php
	...
	while($row=sql_fetch_array($res)) {
		...
		$href1 = "?code=minishop_category_list&sel_ca1={$row['catecode']}";
	...
	?>
		<div>
			...
			<?php if($row['p_oper'] != 'y') { ?>
			<a href="javascript:post_delete('./minishop_category_delete.php', '<?php echo $row['index_no']; ?>');" class="btn_ssmall red">삭제</a>
			<?php } else { ?>
	...
	?>
		<dt>
			...
			<a href="javascript:post_delete('./minishop_category_delete.php', '<?php echo $row2['index_no']; ?>');" class="btn_ssmall red">삭제</a>
			<?php } else { ?>
			<a href="javascript:alert('개별 카테고리만 삭제 가능합니다.');" class="btn_ssmall red">삭제</a>
			<?php } ?>
	...
	?>
		<dd>
			...
			<a href="javascript:post_delete('./minishop_category_delete.php', '<?php echo $row3['index_no']; ?>');" class="btn_ssmall red">삭제</a>
			<?php } else { ?>
			<a href="javascript:alert('개별 카테고리만 삭제 가능합니다.');" class="btn_ssmall red">삭제</a>
			<?php } ?>
	...
	?>
		<dd>
			...
			<a href="javascript:post_delete('./minishop_category_delete.php', '<?php echo $row4['index_no']; ?>');" class="btn_ssmall red">삭제</a>
			<?php } else { ?>
			<a href="javascript:alert('개별 카테고리만 삭제 가능합니다.');" class="btn_ssmall red">삭제</a>
			<?php } ?>
	...
	?>
		<dd>
			...
			<a href="javascript:post_delete('./minishop_category_delete.php', '<?php echo $row5['index_no']; ?>');" class="btn_ssmall red">삭제</a>
			<?php } else { ?>
			<a href="javascript:alert('개별 카테고리만 삭제 가능합니다.');" class="btn_ssmall red">삭제</a>
			<?php } ?>
...
function modok(no)
{
	document.all['cos'+no].src = "./minishop_category_mod.php?index_no="+no;
