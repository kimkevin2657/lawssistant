...
if(!$config['pf_expire_use'])
	alert('관리비 사용이 중지되었습니다.', './page.php?code=minishop_info');
...
$sql_common = " from shop_minishop_term ";
...
$sql2 = " select count(*) as cnt 
		    from shop_minishop_term 
		   where mb_id = '{$member['id']}' 
		     and state = '1' ";
$term = sql_fetch($sql2);
$term_count = (int)$term['cnt'] + 1;

$pb = get_minishop_basic($member['grade']);
?>
...
<form name="ftermlist" id="ftermlist" method="post" action="./minishop_term_delete.php" onsubmit="return ftermlist_submit(this);">
...
function fregform_submit(f)
{
	...
	f.action = "./minishop_term_update.php";
    return true;
}
