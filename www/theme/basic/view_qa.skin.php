...
<tr>
	...
	<td class="tar">
		<a href="<?php echo MS_SHOP_URL; ?>/qaform.php?gs_id=<?php echo $index_no; ?>" onclick="win_open(this,'qaform','600','600','yes');return false" class="btn_lsmall grey">상품Q&A작성</a>
		<a href="<?php echo MS_BBS_URL; ?>/qna_list.php" target="_blank" class="btn_lsmall grey">고객센터 문의하기</a>
	</td>
</tr>
</table>
</form>
...
<div class="tbl_head01 tbl_wrap">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if(substr($row['iq_time'],0,10) == MS_TIME_YMD) {
			$iq_subject .= '&nbsp;<img src="'.MS_IMG_URL.'/icon/icon_new.gif" alt="new">';
		}

		$is_secret = false;
		if($row['iq_secret']) {
			$icon_secret = '<img src="'.MS_IMG_URL.'/icon/icon_secret.jpg" alt="비밀글">';
		...
		if($row['iq_answer']) {
			$icon_answer = '<img src="'.MS_IMG_URL.'/icon/icon_answer.jpg" alt="답변완료">';
		} else {
			$icon_answer = '<img src="'.MS_IMG_URL.'/icon/icon_standby.jpg" alt="미답변">';
		}
	...
	<div id="sod_qa_con_<?php echo $i; ?>" class="sod_qa_con" style="display:none;">
		...
		<tr>
			<td class="vat tal padt10 padb10 padl5 padr5"><img src="<?php echo MS_IMG_URL; ?>/sub/FAQ_Q.gif" align="absmiddle"></td>
			<td class="vat tal padt10 padb10 padl5 padr5">
				<?php echo nl2br($row['iq_question']); ?>
				<?php if(is_admin() || $member['id' ] == $row['mb_id'] && !$iq_answer) { ?>
				<div class="padt10"><a href="<?php echo MS_SHOP_URL; ?>/qaform.php?gs_id=<?php echo $row['gs_id']; ?>&iq_id=<?php echo $row['iq_id']; ?>&w=u" onclick="win_open(this,'upd','600','530','yes');return false"><span class="tu">수정</span></a>&nbsp;<a href="<?php echo MS_SHOP_URL; ?>/qaform_update.php?gs_id=<?php echo $row['gs_id']; ?>&iq_id=<?php echo $row['iq_id']; ?>&w=d&hash=<?php echo $hash; ?>" class="itemqa_delete"><span class="tu">삭제</span></a></div>
				<?php } ?>
			</td>
		</tr>
		<?php if($iq_answer) { ?>
		<tr>
			<td class="vat tal padt10 padb10 padl5 padr5"><img src="<?php echo MS_IMG_URL; ?>/sub/FAQ_A.gif" align="absmiddle"></td>
