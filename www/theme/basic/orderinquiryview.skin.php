...
<div id="con_lf">
	...
	<div class="tbl_head02 tbl_wrap">
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			for($k=0; $ct=sql_fetch_array($res); $k++) {
				...
				$href = MS_SHOP_URL.'/view.php?index_no='.$rw['gs_id'];
		...
		<tr>
			<td class="tac" rowspan="<?php echo $rowspan; ?>">
				...
				<p class="padt5"><a href="<?php echo MS_SHOP_URL; ?>/orderprint.php?od_id=<?php echo $rw['od_id']; ?>" onclick="win_open(this,'winorderprint','670','600','yes');return false;" class="btn_small bx-white"><i class="fa fa-print fs14 vam marb2 marr3"></i> 인쇄</a></p>
			</td>
		</tr>
		<?php } ?>
		<tr class="rows">
				...
				<div class="ini_wrap">
					...
					<tr>
						...
						<td class="vat tal">
							...
							<?php if($rw['dan'] == 5) { $rd = get_review($od['od_id']);
								if(!$rd['od_id']) { ?>
							<p class="padt3"><a href="<?php echo MS_SHOP_URL; ?>/orderreview.php?gs_id=<?php echo $rw['gs_id']; ?>&od_id=<?php echo $rw['od_id']; ?>" onclick="win_open(this, 'winorderreview', '650', '530','yes');return false;" class="btn_ssmall bx-white">구매후기 작성</a></p>
							<?php }} ?>
						</td>
					</tr>
					</table>
			...
			<td class="tac">
				...
				<?php } ?>
				<?php if($rw['dan'] == '2' and $rw['paymethod'] == "신용카드"){
						if($totalcountc == '1'){ ?>
						<p class="padt3"><a href="#" onclick="fcancel_check2();" class="btn_small">주문취소</a></p>
					<?php }else{ ?>
						<p class="padt3"><a href="<?php echo MS_SHOP_URL; ?>/orderinquirycancel_bubun.php?od_no=<?php echo $ct['od_no']; ?>" onclick="return confirm('주문취소를 하시겠습니까 ?');" class="btn_small">주문취소</a></p>
					<?php  }?>
				<?php } ?>
			</td>
		</tr>
	...
	<dl id="sod_ws_tot">
		...
		<?php } ?>
		<?php if($stotal['salebaesong_price']) { ?>
		<dt>배송비할인</dt>
		<dd><strong>-<?php echo display_price($stotal['salebaesong_price']); ?></strong></dd>
		<?php } ?>
		<?php if($stotal['usepoint']) { ?>
		<dt>블링페이결제</dt>
		<dd><strong><?php echo display_point($stotal['usepoint']); ?></strong></dd>
		<?php } ?>
		<?php if($stotal['useppay']) { ?>
		<dt>마일리지결제</dt>
		<dd><strong><?php echo display_point($stotal['useppay']); ?></strong></dd>
		<?php } ?>
        <?php if($stotal['usesppoint']) { ?>
        <dt>쇼핑페이결제</dt>
        <dd><strong><?php echo display_point($stotal['usesppoint']); ?></strong></dd>
        <?php } ?>
		<dt class="ws_price">실결제금액</dt>
		<dd class="ws_price"><strong><?php echo display_price($stotal['useprice']); ?></strong></dd>
		<dt class="bt_nolne">마일리지적립</dt>
		<dd class="bt_nolne"><strong><?php echo str_replace("P","원",display_point($stotal['kv_point'])); ?></strong></dd>
		<dt class="bt_nolne">블링페이적립</dt>
		<dd class="bt_nolne"><strong><?php echo display_point($stotal['point']); ?></strong></dd>
        <!-- <dt class="bt_nolne">쇼핑페이적립</dt>
        <dd class="bt_nolne"><strong><?php echo display_point($stotal['sp_point']); ?></strong></dd> -->
	</dl>
		...
		<div class="tbl_frm01 tbl_wrap">
			...
			<tr>
				...
				<td>
					...
					{
						if($od['od_pg'] == 'lg') {
							require_once MS_SHOP_PATH.'/settle_lg.inc.php';
							$LGD_TID      = $od['od_tno'];
							...
							$hp_receipt_script = 'window.open(\''.MS_BILL_RECEIPT_URL.'mcash_bill&tno='.$od['od_tno'].'&order_no='.$od['od_id'].'&trade_mony='.$stotal['useprice'].'\', \'winreceipt\', \'width=500,height=690,scrollbars=yes,resizable=yes\');';
						}
					?>
					...
					if($od['paymethod'] == '신용카드')
					{
						if($od['od_pg'] == 'lg') {
							...
							$card_receipt_script = 'window.open(\''.MS_BILL_RECEIPT_URL.'card_bill&tno='.$od['od_tno'].'&order_no='.$od['od_id'].'&trade_mony='.$stotal['useprice'].'\', \'winreceipt\', \'width=470,height=815,scrollbars=yes,resizable=yes\');';
						}
					?>
			...
			<tr>
				...
				if($od['od_cash'])
				{
					if($od['od_pg'] == 'lg') {
						...
						require_once MS_SHOP_PATH.'/settle_kcp.inc.php';

						$cash = unserialize($od['od_cash_info']);
						$cash_receipt_script = 'window.open(\''.MS_CASH_RECEIPT_URL.$default['de_kcp_mid'].'&orderid='.$od_id.'&bill_yn=Y&authno='.$cash['receipt_no'].'\', \'taxsave_receipt\', \'width=360,height=647,scrollbars=0,menus=0\');';
					}
				?>
				...
				?>
					<a href="javascript:;" onclick="window.open('<?php echo MS_SHOP_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');" class="btn_small">현금영수증 발급하기</a>
				<?php } ?>
				</td>
			</tr>
			<?php
	...
	<section id="sod_fin_cancel">
		...
		<div id="sod_fin_cancelfrm">
			<form method="post" action="<?php echo MS_SHOP_URL; ?>/orderinquirycancel.php" name="chi" onsubmit="return fcancel_check(this);">
