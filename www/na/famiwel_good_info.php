...
$url = 'http://api.famiwel.co.kr/_prozn/_system/connect_data/good_info_json.php'; //접속할 url 입력
...
$sql = " select * from famiwel_shop_list where use_r = '1' order by server_update_date asc limit 5";
...
//	echo $op_count;
	...
	if($catecode){	
$c_supply_price = ceil($row['supply_price']/((100 - $margin)/100)/10)*10;
...
$del_result = sql_query($del_sql);
		...
		for($i = 0; $i < $op_count; $i++){ // 판매가 가장 낮은 가격 책정하기
			if(!$temp_goods_price){
				$temp_goods_price  = conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - $margin)/100)/10)*10);
			}else{
				$pricepice = '';
				$pricepice = ceil($row['op_products'][$i]['op_supply_price']/((100 - $margin)/100)/10)*10;
		...
		for($i = 0; $i < $op_count; $i++){
			...
			$value['io_price']				= conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - $margin)/100)/10)*10);
...
#################공급사 대기 추가 ##################
		...
		$row3 = sql_fetch(" select * from shop_goods_option where io_famiwel_no='$opopno' "); //db에 등록되었었는지 여부 검사
			...
			if($row3['io_no']){
				unset($opp_supply_price); //기본공급가
				unset($io_supply_price); //옵션공급가
				unset($io_price); //옵션판매가
				unset($io_supply_price_min); //옵션공급가와 기존공급가의 차액
				$opp_supply_price = $row2['supply_price'];
				$io_supply_price = conv_number($row['op_products'][$i]['op_supply_price']) - $opp_supply_price;
				$io_price = $row3['io_price'];
				unset($value);				
				
				if($row3['io_supply_price'] > $io_supply_price){
					$io_supply_price_min = $row3['io_supply_price'] - $io_supply_price;
					//$value['io_price'] = $io_price - $io_supply_price_min; //수식은 넣어놓지만 반영안함 @내용 : 기존 공급가가 새로운 공급가보다 높을때 기존 판매가를 바뀐공급가 차액만큼 빼줌
				}elseif($row3['io_supply_price'] < $io_supply_price){
					$io_supply_price_min = $io_supply_price - $row3['io_supply_price'];
					$value['io_price'] = $io_price + $io_supply_price_min; //@내용 : 기존 공급가가 새로운 공급가보다 낮을때 기존 판매가를 바뀐공급가 차액만큼 더해줌
				}
				...
				$value['gs_id']					= $row2['index_no'];
				$value['io_supply_price']		= conv_number($row['op_products'][$i]['op_supply_price']) - $opp_supply_price;
				$value['io_price']				= conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - $margin)/100)/10)*10) - $opp_price;
				$value['io_stock_qty']			= conv_number($row['op_products'][$i]['op_jego']);
				$value['io_noti_qty']			= 0;
				$value['io_use']				= 1;
				$value['io_famiwel_use']		= $regdater;
