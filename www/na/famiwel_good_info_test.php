...
$sql = " select * from famiwel_shop_list where gid = '16064589477000'";
...
$del_result = sql_query($del_sql);
		...
		for($i = 0; $i < $op_count; $i++){ // 판매가 가장 낮은 가격 책정하기
			if(!$temp_goods_price){
				$temp_goods_price  = conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - 15)/100)/10)*10);
			}else{
				$pricepice = '';
				$pricepice = ceil($row['op_products'][$i]['op_supply_price']/((100 - 15)/100)/10)*10;
		...
		for($i = 0; $i < $op_count; $i++){
			...
			$value['io_price']				= conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - 15)/100)/10)*10);
...
//			update("shop_goods_cate", $value," where gs_id = '{$row2['index_no']}'");
		...
		$row3 = sql_fetch(" select * from shop_goods_option where io_famiwel_no='$opopno' "); //db에 등록되었었는지 여부 검사
			...
			if($row3['io_no']){
				...
				$value['gs_id']					= $row2['index_no'];
				$value['io_supply_price']		= conv_number($row['op_products'][$i]['op_supply_price']) - $opp_supply_price;
				$value['io_price']				= conv_number(ceil($row['op_products'][$i]['op_supply_price']/((100 - 15)/100)/10)*10) - $opp_price;
				$value['io_stock_qty']			= conv_number($row['op_products'][$i]['op_jego']);
				$value['io_noti_qty']			= 0;
				$value['io_use']				= 1;
				$value['io_famiwel_use']		= $regdater;
