...
<ul class="bo_cate" id="bo_cate">
	<li><a href="<?php echo MS_BBS_URL; ?>/list.php?<?php echo $qstr1; ?>"<?php if(!$ca_name) { echo ' id="bo_cate_on"'; } ?>>전체</a></li>
	<?php
	for($i=0; $i<count($usecate); $i++) {
		$active = '';
		if($ca_name == $usecate[$i])
			$active = ' id="bo_cate_on"';
	?>
	<li><a href="<?php echo MS_BBS_URL; ?>/list.php?<?php echo $qstr1; ?>&ca_name=<?php echo $usecate[$i]; ?>"<?php echo $active; ?>><?php echo $usecate[$i]; ?></a></li>
	<?php } ?>
</ul>
<?php } ?>
