...
<ul class="bo_cate">
	<li<?php if(!$ca_name) { echo ' class="active"'; } ?>><a href="<?php echo MS_BBS_URL; ?>/list.php?<?php echo $qstr1; ?>">전체</a></li>
	<?php
	for($i=0; $i<count($usecate); $i++) {
		...
		if($ca_name == $usecate[$i])
			$active = ' class="active"';
	?>
	<li<?php echo $active; ?>><a href="<?php echo MS_BBS_URL; ?>/list.php?<?php echo $qstr1; ?>&ca_name=<?php echo $usecate[$i]; ?>"><?php echo $usecate[$i]; ?></a></li>
	<?php } ?>
</ul>
<?php } ?>
	...
	<fieldset style="padding:0 10px;">
		...
		<tr height="30">
			<td class="tal padr10 padl10">
				...
				<span class="fc_197 marl5">내 동네 매물 검색하기</span>
			</td>
		</tr>
		</table>
		</form>
...
<div class="gallery">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['classification'] == 'apartment'){
			$classification = "아파트/오피스텔/분양";
		}elseif($row['classification'] == 'villa'){
			$classification = "빌라/주택";
		}elseif($row['classification'] == 'oneroom'){
			$classification = "원룸/투룸";
		}elseif($row['classification'] == 'shopping'){
			$classification = "상가/업무/공장/토지";
		}
		...
		if($board['use_category'] == '1'  && $row['ca_name']) {
			$bo_subject .= '<strong>['.$row['ca_name'].']['.$classification.']</strong>&nbsp;';
		}
...
<div class="page_wrap">
	...
	<div class="rbt_box">
		<a href="./write.php?boardid=<?php echo $boardid; ?>" class="btn_lsmall">매물등록하기</a>
	</div>
	<?php } ?>
</div>
</form>
