...
if(MS_IS_MOBILE) {
	goto_url(MS_MBBS_URL.'/board_read.php?boardid='.$boardid.'&index_no='.$index_no);
}
...
if($bo_file1)
	$refile1 = "<a href='".MS_DATA_URL."/board/$boardid/$bo_file1' target='_blank'>$bo_file1</a>";

if($bo_file2)
	$refile2 = "<a href='".MS_DATA_URL."/board/$boardid/$bo_file2' target='_blank'>$bo_file1</a>";
	...
	if(!is_admin())
	{
		...
		else
		{
			...
			if($inpasswd != $bo_passwd) {
				goto_url(MS_BBS_URL."/secret.php?index_no=$index_no&$qstr1");
			}
			else
			{
				//관리자가 답변을 달면 본인이 글을 볼수가 없었던문제 버그수정 jck
...
$from_action_url = MS_HTTPS_BBS_URL."/tail_write.php";

include_once(MS_BBS_PATH."/skin/{$board['skin']}/read.php");
