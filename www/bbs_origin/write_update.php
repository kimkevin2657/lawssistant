...
if ($w == '' || $w == 'u') {
    ...
    if($w =='u' && $member['id'] && $wr['mb_id'] === $member['id']) {
        ;
...
if ($w == '' || $w == 'r') {
    ...
    if ($member['id']) {
        $mb_id = $member['id'];
        $wr_name = addslashes(clean_xss_tags($board['bo_use_name'] ? $member['name'] : $member['mb_nick']));
        $wr_password = '';
    ...
    // 새글 INSERT
    //sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id ) values ( '{$bo_table}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$member['id']}' ) ");
    ...
    if ($w == '') {
        ...
        insert_point($member['id'], $board['bo_write_point'], "{$board['bo_subject']} {$wr_id} 글쓰기", $bo_table, $wr_id, '쓰기');
    } else {
        // 답변은 코멘트 포인트를 부여함
        // 답변 포인트가 많은 경우 코멘트 대신 답변을 하는 경우가 많음
        insert_point($member['id'], $board['bo_comment_point'], "{$board['bo_subject']} {$wr_id} 글답변", $bo_table, $wr_id, '쓰기');
    }
}  else if ($w == 'u') {
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['id'] != $group['gr_admin']) // 자신이 관리하는 그룹인가?
            alert('자신이 관리하는 그룹의 게시판이 아니므로 수정할 수 없습니다.', $return_url);
        else if ($member['grade'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            alert('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.', $return_url);
    } else if ($is_admin == 'board') { // 게시판관리자이면
        $mb = get_member($write['mb_id']);
        if ($member['id'] != $board['bo_admin']) // 자신이 관리하는 게시판인가?
            alert('자신이 관리하는 게시판이 아니므로 수정할 수 없습니다.', $return_url);
        else if ($member['grade'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            alert('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.', $return_url);
    } else if ($member['id']) {
        if ($member['id'] != $write['mb_id'])
...
// 사용자 코드 실행
//echo MS_BBS_URL.'/'.$board_skin_path.'/write_update.skin.php';
if($bo_table == "approval"){
    @include_once('/home/pulo/www/bbs/skin/basic_doc/write_update.skin.php');
    @include_once('/home/pulo/www/bbs/skin/basic_doc/write_update.tail.skin.php');
}
...
if ($file_upload_msg)
    ...
    goto_url(MS_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);