...
//echo "G5_EDITOR_LIB".G5_EDITOR_LIB;
...
if ($w == '') {
    ...
    /* if ($member['grade'] < $board['bo_write_level']) {
        if ($member['id']) {
            alert('글을 쓸 권한이 없습니다.');
        } else {
    ...
    //if ($member['grade'] < $board['bo_write_level']) {
    if($member['id'] && $write['mb_id'] === $member['id']) {
        ;
    } else if ($member['grade'] < $board['bo_write_level']) {
        if ($member['id']) {
    ...
    if ($member['grade'] < $board['bo_reply_level']) {
        if ($member['id'])
    ...
    if (strstr($write['wr_option'], 'secret')) {
        if ($write['mb_id']) {
            ...
            if (!($write['mb_id'] === $member['id'] || $is_admin))
...
if (!empty($group['gr_use_access'])) {
    ...
    if ($is_admin == 'super' || $group['gr_admin'] === $member['id'] || $board['bo_admin'] === $member['id']) {
        ; // 통과
    } else {
        // 그룹접근
        $sql = " select gr_id from {$g5['group_member_table']} where gr_id = '{$board['gr_id']}' and mb_id = '{$member['id']}' ";
        $row = sql_fetch($sql);
        if (!$row['gr_id'])
...
$is_html = false;
if ($member['grade'] >= $board['bo_html_level'])
    $is_html = true;
...
$is_email    = false;
$is_homepage = false;
if ($is_guest || ($is_admin && $w == 'u' && $member['id'] !== $write['mb_id'])) {
    $is_name = true;
    $is_password = true;
    $is_email = true;
    $is_homepage = true;
...
$is_link = false;
if ($member['grade'] >= $board['bo_link_level']) {
    $is_link = true;
}

$is_file = false;
if ($member['grade'] >= $board['bo_upload_level']) {
    $is_file = true;
}
...
if ($w == '') {
    ...
    if (!$is_admin) {
        if (!($is_member && $member['id'] === $write['mb_id'])) {
...
/* if ($config['cf_editor'] && $is_dhtml_editor_use && $board['bo_use_dhtml_editor'] && $member['grade'] >= $board['bo_html_level']) {
    $is_dhtml_editor = true;

    if ( $w == 'u' && (! $is_member || ! $is_admin || $write['mb_id'] !== $member['id']) ){
...
//echo G5_EDITOR_PATH.'/smarteditor3/autosave.editor.js';

/* if(is_file(G5_EDITOR_PATH.'/smarteditor3/autosave.editor.js')){
    $editor_content_js = '<script src="'.G5_EDITOR_URL.'/smarteditor3/autosave.editor.js"></script>'.PHP_EOL;
}else{
    echo "bbbb";
}
 */
/* echo "js".$editor_content_js."<br>";
echo "editor_html:".$is_dhtml_editor."<br>";
...
// 임시 저장된 글 수
//$autosave_count = autosave_count($member['id']);

include_once(G5_PATH.'/head.sub.php');
if($bo_table == "approval"){
    @include_once ('/home/pulo/www/bbs/skin/basic_doc/write.head.skin.php');
}
...
if($bo_table == "approval"){
    include_once ('/home/pulo/www/bbs/skin/basic_doc/write.skin.php');
}else if($bo_table == "schedule"){
    include_once ('/home/pulo/www/bbs/skin/schedule/write.skin.php');
}

include_once('./board_tail.php');
if($bo_table == "approval"){
    @include_once ('/home/pulo/www/bbs/skin/basic_doc/write.tail.skin.php');
}
