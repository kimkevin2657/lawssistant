...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        ...
        if ($is_admin == "super" || $is_admin == "group") {
            ...
            // 그룹접근
            $sql = " select count(*) as cnt from {$g5['group_member_table']} where gr_id = '{$board['gr_id']}' and mb_id = '{$member['id']}' ";
            $row = sql_fetch($sql);
            if (!$row['cnt']) {
    ...
    // 자신의 글이거나 관리자라면 통과
    if (($write['mb_id'] && $write['mb_id'] === $member['id']) || $is_admin) {
        ;
    } else {
        // 비밀글이라면
    ...
    if (!get_session($ss_name))
    {
        ...
        // 자신의 글이면 통과
        if ($write['mb_id'] && $write['mb_id'] === $member['id']) {
            ;
            ...
            insert_point($member['id'], $board['bo_read_point'], ((G5_IS_MOBILE && $board['bo_mobile_subject']) ? $board['bo_mobile_subject'] : $board['bo_subject']).' '.$wr_id.' 글읽기', $bo_table, $wr_id, '읽기');
        }
...
$admin_href = "";
// 최고관리자 또는 그룹관리자라면
if ($member['id'] && ($is_admin === 'super' || $group['gr_admin'] === $member['id']))
