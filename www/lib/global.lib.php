...
function is_mobile()
{
	return preg_match('/'.MS_MOBILE_AGENT.'/i', $_SERVER['HTTP_USER_AGENT']);
}
...
function delete_board_thumbnail($boardid, $file)
{
    ...
    $files = glob(MS_DATA_PATH.'/board/'.$boardid.'/thumb-'.$fn.'*');
    if (is_array($files)) {
...
function check_string($str, $options)
{
    ...
    for($i=0;$i<strlen($str);$i++) {
        ...
        // 숫자
            ...
            if($options & MS_NUMERIC) {
                $s .= $c;
            }
        }
        // 영대문자
        else if($oc >= 0x41 && $oc <= 0x5A) {
            if(($options & MS_ALPHABETIC) || ($options & MS_ALPHAUPPER)) {
                $s .= $c;
            }
        }
        // 영소문자
        else if($oc >= 0x61 && $oc <= 0x7A) {
            if(($options & MS_ALPHABETIC) || ($options & MS_ALPHALOWER)) {
                $s .= $c;
            }
        }
        // 공백
...
{
    $f = file(MS_PLUGIN_PATH.'/htmlpurifier/safeiframe.txt');
    $domains = array();
    foreach($f as $domain){
        // 첫행이 # 이면 주석 처리
    ...
    include_once(MS_PLUGIN_PATH.'/htmlpurifier/HTMLPurifier.standalone.php');
    $config = HTMLPurifier_Config::createDefault();
    // data/cache 디렉토리에 CSS, HTML, URI 디렉토리 등을 만든다.
    $config->set('Cache.SerializerPath', MS_DATA_PATH.'/cache');
...
function rm_rf($file)
{
    if(file_exists($file)) {
        if(is_dir($file)) {
            ...
            @chmod($file, MS_DIR_PERMISSION);
            @rmdir($file);
        } else {
            @chmod($file, MS_FILE_PERMISSION);
            @unlink($file);
        }
    }
}
...
// 동일한 host url 인지
function check_url_host($url, $msg='', $return_url=MS_URL)
{
    if(!$msg)
...
{
    if(defined('MS_STRING_ENCRYPT_FUNCTION') && MS_STRING_ENCRYPT_FUNCTION) {
        $encrypt = call_user_func(MS_STRING_ENCRYPT_FUNCTION, $str);
    } else {
...
function get_sns_share_link($sns, $url, $title, $image_url)
{
	...
	$sns_send = MS_LIB_URL.'/sns_send.php?longurl='.urlencode($sns_url).'&amp;title='.urlencode($sns_msg);
			...
			switch($cp['cp_type']){
					...
					if(($cp['cp_pub_sdate'] <= MS_TIME_YMD || $cp['cp_pub_sdate'] == '9999999999') &&
					   ($cp['cp_pub_edate'] >= MS_TIME_YMD || $cp['cp_pub_edate'] == '9999999999')) {
						...
						for($j=0; $j<count($cp_week_day); $j++) {
							for($k = 1; checkdate(MS_TIME_MONTH, $k, MS_TIME_YEAR); $k ++) {
								$thismonth = MS_TIME_MONTH.'/'.substr(sprintf('%02d',$k), -2);
						...
						$cnt = substr_count($is_week, MS_TIME_DAY);
						if($cnt) {
							//쿠폰발행 시간
							$tmp_cpdown = array();
							for($j=1; $j<=3; $j++) {
				...
				case '3': // 회원 생일자 발급
					...
					if($is_check_vars) {
						...
						if(MS_TIME_YMD >= $fr_birthday && MS_TIME_YMD <= $to_birthday) {
							$tmp_coupon[] = $cp['cp_id'];
						}
					}
					break;
...
// DB 연결
function sql_connect($host, $user, $pass, $db=MS_MYSQL_DB)
{
    if(function_exists('mysqli_connect') && MS_MYSQLI_USE) {
...
{
    if(function_exists('mysqli_select_db') && MS_MYSQLI_USE)
...
function sql_set_charset($charset, $link=null)
{
    ...
    if(function_exists('mysqli_set_charset') && MS_MYSQLI_USE)
...
function sql_query($sql, $error=MS_DISPLAY_SQL_ERROR, $link=null)
{
    global $ms, $_sql_log;
    ...
    if(function_exists('mysqli_query') && MS_MYSQLI_USE) {
        if($error) {
    ...
    if( is_array($_sql_log) ) array_push($_sql_log, $sql);
    return $result;
}

// 쿼리를 실행한 후 결과값에서 한행을 얻는다.
function sql_fetch($sql, $error=MS_DISPLAY_SQL_ERROR, $link=null)
{
    global $ms;
...
{
    if(function_exists('mysqli_fetch_assoc') && MS_MYSQLI_USE)
...
{
    if(function_exists('mysqli_free_result') && MS_MYSQLI_USE)
...
function sql_insert_id($link=null)
{
    ...
    if(function_exists('mysqli_insert_id') && MS_MYSQLI_USE)
...
{
    if(function_exists('mysqli_num_rows') && MS_MYSQLI_USE)
...
{
    if(function_exists('mysqli_fetch_row') && MS_MYSQLI_USE)
...
function sql_field_names($table, $link=null)
{
    ...
    if(function_exists('mysqli_fetch_field') && MS_MYSQLI_USE) {
...
function sql_error_info($link=null)
{
    ...
    if(function_exists('mysqli_error') && MS_MYSQLI_USE) {
...
function insert($table,$values)
{
    ...
    if( $table == 'shop_member' ){
        Member::updateFamilyInfo($values['id']);
    }
...
function insert_search($table,$values)
{
	...
	if(strpos($_SERVER['HTTP_REFERER'], 'https://ownermarket.co.kr') !== false) {
		$sql = "insert into $table ($field) VALUES ($value)";	// 실제 쿼리 생성
		return sql_query($sql);
	}
}
...
function icode_sms_send($mb_id, $fld)
{
	...
	$minishop_hp = ($mb['pt_id'] != encrypted_admin()) ? $pt['cellphone'] : '';
	...
	if($sm["cf_mb_use{$fld}"] || $sm["cf_ad_use{$fld}"] || $sm["cf_re_use{$fld}"])
	{
		...
		if($sm['cf_icode_id'] && $sm['cf_icode_pw']) {
			...
			if($userinfo['code'] == 0) {
				if($userinfo['payment'] == 'C') { // 정액제
					...
					$minimum_coin = 100;
					if(defined('MS_ICODE_COIN'))
						$minimum_coin = intval(MS_ICODE_COIN);
		...
		if($is_sms_send)
		{
			...
			$recv_numbers = array($mb_hp, $admin_hp, $minishop_hp);
			...
			if($sms_count > 0) {
				...
				if($sm['cf_sms_type'] == 'LMS') {
					include_once(MS_LIB_PATH.'/icode.lms.lib.php');
					...
					include_once(MS_LIB_PATH.'/icode.sms.lib.php');
...
function icode_order_sms_send($od_hp, $fld, $od_id)
{
	...
	$minishop_hp = ($od['pt_id'] != encrypted_admin()) ? $pt['cellphone'] : '';
	...
	if($sm["cf_mb_use{$fld}"] || $sm["cf_ad_use{$fld}"] || $sm["cf_re_use{$fld}"] || $sm["cf_sr_use{$fld}"])
	{
		...
		if($is_sms_send)
		{
			...
			$recv_numbers = array($od_hp, $admin_hp, $minishop_hp, $seller_hp);
			$send_number = conv_number($sm['cf_sms_recall']);
			...
			for($s=0; $s<count($sms_send_use); $s++) {
				...
				$sms_content = rpc($sms_content, "{상품명}", $gs['gname']);
...
function member_delete($mb_id)
{
	...
	$banner_dir = MS_DATA_PATH."/banner";
	$goods_dir  = MS_DATA_PATH."/goods";
	...
	while($row = sql_fetch_array($res)) {
		...
		$sql = "insert into shop_leave_log ( new_id, old_id, mb_id, reg_time, memo )
				values ( '$mb[pt_id]','$mb_id','$row[id]','".MS_TIME_YMDHIS."','$memo' )";
		sql_query($sql);
	}
	...
	sql_query("delete from shop_minishop where mb_id='$mb_id'"); // 가맹점정보
	sql_query("delete from shop_minishop_pay where mb_id='$mb_id'"); // 가맹점 수수료
	sql_query("delete from shop_minishop_payrun where mb_id='$mb_id'"); // 가맹점 출금요청
	sql_query("delete from shop_minishop_term where mb_id='$mb_id'"); // 가맹점 기간연장
	...
	sql_query("delete from shop_point where mb_id='$mb_id'"); // 회원 쇼핑포인트
...
// 정회원의 정보를 리턴
function get_member_pt($mb_id, $fileds='*')
{
	return sql_fetch("select $fileds from shop_member where id = TRIM('$mb_id') and (grade ='2' or grade ='3' or grade ='4' or grade ='5' or grade ='6' or grade = '1')");
}

// 정회원의 정보를 리턴
function get_member_pt_name($mb_id, $fileds='*')
{
	return sql_fetch("select $fileds from shop_member where pt_id_name = TRIM('$mb_id') and (grade ='2' or grade ='3' or grade ='4' or grade ='5' or grade ='6' or grade = '1')");
}
...
// 회원의 이름을 리턴
function get_member_name($mb_id, $fileds='*')
{
	$LYJ = sql_fetch("select name from shop_member where id = TRIM('$mb_id')");
  return $LYJ['name'];
}
...
{
	$skin_path = MS_PATH.'/m/theme/'.$skin;
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='')
{
    ...
    // 쇼핑포인트를 사용한 경우 쇼핑포인트 내역에 사용금액 기록
    if($point < 0) {
...
function get_sql_search($word, $pt_id)
{
	...
	if(!file_exists(MS_PATH."/DEMO")) {
		...
		if(!$row['index_no']) {
			$value['scount'] = 1;
			$dddd = insert_search("shop_keyword",$value);
			echo $dddd;
		} else {
...
function get_it_image($gs_id, $it_img, $wpx, $hpx=0, $img_id='')
{
	...
	{
		$file = MS_DATA_PATH."/goods/".$it_img;
		if(is_file($file) && $it_img)
		{
			...
			if($img_wpx != $wpx && $img_hpx != $hpx) {
				...
				if($filename) {
					$savepath = MS_DATA_PATH."/goods/".$gs['gcode'];
					$size = @getimagesize($file);
					// Animated GIF는 썸네일 생성하지 않음
					if($size[2] == 1) {
						if(is_animated_gif($file))
							$savepath = MS_DATA_PATH."/goods";
					}
					$thumb = @thumbnail($filename, $filepath, $savepath, $wpx, $hpx, false, true, 'center', false, $um_value='80/0.5/3', false);
				}

				$file_url = rpc($savepath, MS_PATH, MS_URL);
			} else {
				$file_url = rpc($filepath, MS_PATH, MS_URL);
			}
			...
			$img = '<img src="'.MS_IMG_URL.'/noimage.gif" width="'.$wpx.'" height="'.$hpx.'"';
		}
	}
	else {
...
function get_od_image($od_id, $it_img, $wpx, $hpx=0)
{
	...
	if(preg_match("/^(http[s]?:\/\/)/", $it_img) == false)
	{
		$file = MS_DATA_PATH."/order/".substr($od_id,0,4)."/".$od_id."/".$it_img;
		if(is_file($file) && $it_img)
		{
            $size = @getimagesize($file);
			$img_wpx  = $size[0];
...
// 회원 레벨
function get_search_biz_type($field, $value, $start_id=1, $end_id=9)
{
    $str = radio_checked($field, $value,  '', '전체');

    $sql = " select *
			   from shop_minishop_type
			  where biz_no between {$start_id} and {$end_id}
			  order by biz_no asc ";
    $result = sql_query($sql);
    for($i=0; $row=sql_fetch_array($result); $i++) {
        $str .= radio_checked($field, $value, $row['biz_type'], $row['biz_type_name']);
    }
...
function display_itemtype($mb_id, $type, $rows='')
{
	...
	$sql = " select distinct a.*
			   from (select a.* ".get_sql_precompose('').") a left join shop_goods_type b on (a.index_no=b.gs_id)
			  where b.mb_id = '$mb_id'
				and a.shop_state = '0'
				and a.isopen = '1'
				and find_in_set('$mb_id', a.use_hide) = '0'
				and b.it_type{$type} = '1'
			  order by b.it_type{$type}_sun desc ";
	if($rows) $sql .= " limit $rows ";
	$result = sql_query($sql);
	$type_count = sql_num_rows($result);
	if(!$type_count && $mb_id != $encrypted_admin) {
		$sql = " select distinct a.*
				   from (select a.* ".get_sql_precompose('').") a left join shop_goods_type b on (a.index_no=b.gs_id)
				  where b.mb_id = '{$encrypted_admin}'
					and a.shop_state = '0'
					and a.isopen = '1'
					and find_in_set('$mb_id', a.use_hide) = '0'
					and b.it_type{$type} = '1'
...
// 상품 진열,1
function display_itemtype_new($mb_id, $type, $rows='')
{
	$sql = " select a.*
...
function query_itemtype($mb_id, $type, $sql_search, $sql_order)
{
    ...
    if( false && $type == '2' ) {
        /**
         * FIXED: 회원쇼핑몰은 전체 가맹상품 제외 진열 되도록 수정 하였습니다.
         */
        $sql = " select distinct a.*
				   from (select a.* ".get_sql_precompose($sql_search).") a left join shop_goods_type b on (a.index_no=b.gs_id)
				  where b.mb_id = '{$encrypted_admin}'
					and a.shop_state = '0'
					and a.isopen = '1'
					and find_in_set('$mb_id', a.use_hide) = '0'
				    and a.buy_minishop_grade = '0'
					{$sql_search}
					{$sql_order} ";
        $result = sql_query($sql);
    } else {
...
function query_itemtype_minishops($mb_id, $type, $sql_search, $sql_order)
{
    global $encrypted_admin;

    $sql = " select a.*
           from (select a.* ".get_sql_precompose($sql_search).") a
          where a.mb_id = '$mb_id'
            and a.shop_state = '0'
            and a.isopen = '1'
            and find_in_set('$mb_id', a.use_hide) = '0'
...
function check_admin_token()
{
    ...
    if(!$token || !$_REQUEST['token'] || $token != $_REQUEST['token'])
        alert('올바른 방법으로 이용해 주십시오.', MS_URL);
        ...
        if(preg_match("/\.(gif|jpg|jpeg|png)$/i", $filename))
		{
            ...
            if(is_file($path)) {
                ...
                if($size[2] < 1 || $size[2] > 3) {
                    ...
                    @chmod($dir.'/'.$filename, MS_FILE_PERMISSION);
                }
            }
        }
    }
...
function get_head_title($fild, $mb_id)
{
	...
	$row = sql_fetch("select $fild from shop_minishop where mb_id='$mb_id' ");
	if(!$row[$fild])
...
{
    sql_query("insert into shop_seller_code ( reg_at ) values( now() )");
    $idx = sql_insert_id();
...
function get_cart_count()
{
	...
	$sql = " select * from shop_cart where ct_direct='{$member['id']}' and ct_select='0' group by gs_id ";
	$result = sql_query($sql);
...
// 금액 표시
function display_price($price, $unit = '원')
{
	return number_format($price, 0).$unit;
}

// 금액 표시
function display_price2($price, $surfix = '')
{
	return '<p class="spr">'.number_format($price).'<span>원</span>'.$surfix.'</p>';
}

// 쇼핑포인트 표시
function display_point($price, $unit = 'P')
{
	return number_format($price, 0).$unit;
}
...
function get_order_spay($od_id, $sql_search='')
{
	$sql = " select SUM(goods_price) as price,
					...
					SUM(use_sp_point) as usesppoint,
					SUM(use_price) as useprice,
					SUM(use_ppay) as useppay,
					SUM(sum_qty) as qty,
					SUM(sum_point) as point,
					SUM(sum_sp_point) as sp_point,
					SUM(goods_kv) as kv_point
			   from shop_order
			  where od_id = '$od_id'
				{$sql_search} ";
	$row = sql_fetch($sql);
...
// 주문정보 리뷰조회
function get_review($order_id, $fileds='*')
{
	$sql_where = " where od_id = '$order_id'"; // 주문일련번호
  $zql = " select $fileds from shop_goods_review {$sql_where} ";

	return sql_fetch($zql);
}
...
function order_delete($od_no, $od_id)
{
	...
	// 주문취소 회원의 쇼핑포인트를 되돌려 줌
	if($od['mb_id'] && $od['use_point']) {
		insert_point($od['mb_id'], $od['use_point'], "주문번호 {$od_id} ({$od_no}) 주문취소");
	}

    // 사용한 회원의 쇼핑페이를 반환
    if($od['mb_id'] && $od['use_sp_point']) {
        insert_shopping_pay($od['mb_id'], $od['use_sp_point'], "주문번호 {$od['od_id']} ({$od_no}) 주문취소", 'order', $od_no, 'delete', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }
...
function add_io_stock($od_no, $od_id)
{
	...
	while($ct=sql_fetch_array($res)) {
		...
		$stock_qty = (int)get_it_stock_qty($ct['gs_id']); // 한정수량 조사
		if($gs['stock_mod'] == '1'){
			if($stock_qty > '0'){
...
function change_status_final($od_no)
{
    ...
    change_order_status_completed($od_no);
...
function change_status_final_cancel($od_no)
{
    ...
    change_order_status_cancel($od_no);
...
function change_order_status_5($od_no)
{
	...
	$sql = " update shop_order
				set dan = '5'
				  , invoice_date = '".MS_TIME_YMDHIS."' ";
	if(is_null_time($od['delivery_date'])) { // 배송일이 비었나?
		$sql .= " , delivery_date = '".MS_TIME_YMDHIS."' ";
	}
 ...
 * @param $od
 */
function change_order_status_completed($od_no)
{
    global $config;
    $od = get_order($od_no);
    $gs = unserialize($od['od_goods']);

    //if( (ORD_DONE_DAN == 'USER_OK' && $od['user_ok'] != '1') || ($od['dan'] != ORD_DONE_DAN) ) return;
	if( (ORD_DONE_DAN == 'USER_OK' && $od['user_ok'] != '1') || ($od['dan'] != 5) ) return;
    ...
    $mb = get_member($od['mb_id'], 'name, id, grade, up_id');
    $grade = Member::get_grade($mb['grade']);
    // 쇼핑포인트 적립
    if ($od['mb_id'] && $od['sum_point'] > 0) {
        insert_point($od['mb_id'], $od['sum_point'], "주문번호 {$od['od_id']} ({$od_no}) 배송완료", "@delivery", $od['mb_id'], "{$od['od_id']},{$od_no}");
        if ($grade['gb_pf_per_point'] > 0)
            insert_point($od['mb_id'], $od['use_point'] / 100 * $grade['gb_pf_per_point'], "주문번호 {$od['od_id']} ({$od_no}) 배송완료({$grade['gb_name']})", "order_reward", $od['mb_id'], "{$od['od_id']},{$od_no}");
    }

    // 회원의 쇼핑페이 재적립
    if (defined('USE_SHOPPING_PAY') && USE_SHOPPING_PAY && $od['mb_id'] && $od['use_price']) {
        $sp_point = $od['use_price'] / 100 * $grade['gb_pf_per_sp_point'];
        insert_shopping_pay($od['mb_id'], $sp_point, "주문번호 {$od['od_id']} ({$od_no}) 배송완료", 'order_reward', $od_no, 'delivery', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }
	...
	// 추천ID + 추천판매수수료
    if (!(empty($gs['up_pay_value']) || empty($gs['up_id']))) {
		// (판매가격 - 공급가격) / 100 * %
        $up_id = $gs['up_id'];
        $pt_pay = $gs['up_pay_unit'] == '%' ? ($od['goods_price'] - $od['supply_price']) / 100 * $gs['up_pay_value'] : $gs['up_pay_value'];
        // 상품을 추천한 회원에게 판매 수수료 지급
		//insert_pay($up_id, $pt_pay, _r('{0}({1})님 구매 추천판매수수료', $od['mb_id'], $mb['name']), 'order', $od['od_no'], 'goods_up_id', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);

		insert_pay($up_id, $pt_pay, "{0}({1})님 구매 추천판매수수료(${od['mb_id']}, ${mb['name']})", 'order', $od['od_no'], 'goods_up_id', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);

    }


    minishop::joinFromOrder(Order::factory($od, $gs));
}

/**
 * @param $od_no
 */
function change_order_status_cancel($od_no)
{
    $od = get_order($od_no);

    if( (ORD_DONE_DAN == 'USER_OK' && $od['user_ok'] != '0') ) return;
    if( ORD_DONE_DAN == 5 && ! in_array($od['dan'], [6,7])) return;
    ...
    // 회원의 쇼핑페이 재적립 반환
    if($od['mb_id'] && $od['use_price']) {
        delete_shopping_pay($od['mb_id'], 'order_reward', $od_no, 'delivery');
    }
    ...
    // 사용한 회원의 쇼핑페이를 반환
    if ($od['mb_id'] && $od['use_sp_point']) {
        insert_shopping_pay($od['mb_id'], $od['use_sp_point'], "주문번호 {$od['od_id']} ({$od_no}) 주문취소", 'order', $od_no, 'cancel', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }

    // 사용한 회원의 마일리지를 반환
    if ($od['mb_id'] && $od['use_ppay']) {
        insert_pay($od['mb_id'], $od['use_ppay'], "주문번호 {$od['od_id']} ({$od_no}) 반품", 'order', $od_no, 'cancel', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }
...
function change_order_status_9($od_no)
{
    ...
    // 사용한 회원의 쇼핑페이를 반환
    if($od['mb_id'] && $od['use_sp_point']) {
        insert_shopping_pay($od['mb_id'], $od['use_sp_point'], "주문번호 {$od['od_id']} ({$od_no}) 환불", 'order', $od_no, 'cancel', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }

    // 사용한 회원의 마일리지를 반환
    if($od['mb_id'] && $od['use_ppay']) {
        insert_pay($od['mb_id'], $od['use_ppay'], "주문번호 {$od['od_id']} ({$od_no}) 환불", 'order', $od_no, 'cancel', $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_USER_AGENT']);
    }
...
function get_delivery_inquiry($company, $invoice, $class='')
{
	...
	$str = '';
	if($com && $url) {
		$str .= '<a href="'.$url.$invoice.'" data-invoice="'.$invoice.'" target="_blank"';
		if($class) $str .= ' class="delivery-search-btn '.$class.'"';
		$str .='>배송조회</a>';
	}
...
function get_catename($code)
{
	...
	$row = sql_fetch("select catename from {$ms['category_table']} where catecode='$code'");
	return $row['catename'];
}

// 베스트 200 카테고리
function get_catebest($code)
{
	global $ms;

	$row = sql_fetch("select best from {$ms['category_table']} where catecode='$code' and best='1'");
  if($row['best']=="1") {
    return "Y";
  } else {
    return "N";
...
function is_free_baesong($row)
{
	...
	if($row['sc_type'] == 0) {
		if($row['mb_id'] == encrypted_admin()) { // 본사
			$delivery_method = $config['delivery_method'];
		} else { // 가맹점 및 공급사
			if($row['use_aff'])
				$sr = get_minishop($row['mb_id'], 'delivery_method');
			else
...
function get_item_sendcost($sell_price){
	...
	// 공통설정
	if($gs['sc_type']=='0') {
		if($gs['mb_id'] == encrypted_admin()) { // 본사
			$delivery_method = $config['delivery_method'];
...
function get_item_tot_sendcost($sell_price,$gs_mb_id,$use_aff){
	Global $config;

	if($gs_mb_id == encrypted_admin()) { // 본사
		$delivery_method = $config['delivery_method'];
		$delivery_price = $config['delivery_price'];
		$delivery_price2 = $config['delivery_price2'];
		$delivery_minimum = $config['delivery_minimum'];
		...
		// 배송비
		if($use_aff){
			$sr = get_minishop($gs_mb_id);
		}else{
			$sr = get_seller_cd($gs_mb_id);
		}
		$delivery_method = $sr['delivery_method'];
		$delivery_price = $sr['delivery_price'];
		$delivery_price2 = $sr['delivery_price2'];
	...
	switch($delivery_method) { // 배송정책
		...
		case '2': // 착불배송
			...
			break;
		case '3': // 유료배송
			$send_cost = $delivery_price;
			break;
		case '4': // 조건부무료배송
			if($sell_price >= $delivery_minimum) {
				$send_cost = 0;
			} else {
				$send_cost = $delivery_price2;
			}
			break;
	}
...
{
    global $ms, $pt_id, $config, $auth_good;
    ...
    // 가맹점 판매권한 체크
    if($auth_good) {
        $sql_minishop = " OR a.mb_id = '$pt_id'";
    }

    $sql  = " FROM shop_goods a
				   LEFT JOIN shop_goods_cate b ON ( a.index_no = b.gs_id )
				   LEFT JOIN {$ms['category_table']} c ON ( b.gcate = c.catecode )
			 WHERE a.shop_state = '0'
			   AND a.isopen = '1'
			   AND c.u_hide = '0'
			   AND (a.use_aff = '0'{$sql_minishop})
...
function get_sale_price2($gs_id)
{
	global $member;

	$gb = sql_fetch("select * from shop_member_grade where gb_no = '$member[grade]'");
	$gs = sql_fetch("select goods_price,use_aff,point_pay_allow,point_pay_point,point_pay_per from shop_goods where index_no = '$gs_id'");
...
function display_logo_url($fld='basic_logo')
{
	...
	if(is_file($file) && $row[$fld]) {
		return rpc($file, MS_PATH, MS_URL);
	}
...
function save_goods_data($gs_id, $odrno, $od_id)
{
	...
	$data = addslashes(serialize($cp));

	// 상품정보를 주문서에 업데이트한다.
	$sql = " update shop_order set od_goods = '$data' where index_no = '$odrno' ";
	sql_query($sql);

	$ymd_dir = MS_DATA_PATH.'/order/'.date('ym', time());
	$upl_dir = $ymd_dir.'/'.$od_id; // 저장될 위치

	// 년도별로 따로 저장
	if(!is_dir($ymd_dir)) {
		@mkdir($ymd_dir, MS_DIR_PERMISSION);
		@chmod($ymd_dir, MS_DIR_PERMISSION);
	}

	// 주문번호별로 따로 저장
	if(!is_dir($upl_dir)) {
		@mkdir($upl_dir, MS_DIR_PERMISSION);
		@chmod($upl_dir, MS_DIR_PERMISSION);
	}

	if(preg_match("/^(http[s]?:\/\/)/", $cp['simg1']) == false)
	{
		$file = MS_DATA_PATH.'/goods/'.$cp['simg1'];
		if(is_file($file) && $cp['simg1']) {
			$file_url = $upl_dir.'/'.$cp['simg1'];
			@copy($file, $file_url);
			@chmod($file_url, MS_FILE_PERMISSION);
		}
	}
}
...
// 단순하게 해결 이미지 추출없이 소스상에서 img 태그 찾아서 클래스만 추가하게 변경함
function get_image_resize($html) {
    // img 태그에서 src 속성을 추출하여 원하는 형태로 수정합니다.
    $fixed_html = preg_replace('/<img src=(.*?)>/', '<img class="img_fix" src="$1">', $html);
    // ',' (쉼표)를 제거합니다.
    $fixed_html = str_replace('，', '', $fixed_html);
...
function get_policy_content($gs_id)
{
	...
	if(true || $gs['mb_id']==encrypted_admin())	{
		if(MS_IS_MOBILE)
...
function sql_member_category($mb_id)
{
	...
	$target_table = 'shop_cate_'.$mb_id;
	$member_path = MS_DATA_PATH.'/category/'.$mb_id;
	$admin_path = MS_DATA_PATH.'/category/admin';

	if(!is_dir($member_path)) {

		// 카테고리 폴더 생성
		@mkdir($member_path, MS_DIR_PERMISSION);
		@chmod($member_path, MS_DIR_PERMISSION);
		...
		*/
		...
		while($entry = $d->read()) {
			...
			if(is_dir($admin_path.'/'.$entry)){
				...
				@mkdir($member_path.'/'.$entry, MS_DIR_PERMISSION);
				@chmod($member_path.'/'.$entry, MS_DIR_PERMISSION);
				while($entry2 = $dd->read()) {
					if($entry2 == '.' || $entry2 == '..') continue;
					@copy($admin_path.'/'.$entry.'/'.$entry2, $member_path.'/'.$entry.'/'.$entry2);
					@chmod($member_path.'/'.$entry.'/'.$entry2, MS_DIR_PERMISSION);
					$copy_file++;
				}
				$dd->close();
			}
	...
	if(!table_exists($target_table)) {
		include MS_ADMIN_PATH."/category/category_sql.php";
...
// 분류 쿼리
function sql_query_cgy($upcate, $type='', $rows='', $level, $category)
{
	global $ms, $config;
	...
	if($level > 1){
		$sql_search .= " AND ( display_level = '{$level}' or display_level = '0' ) ";
	}else{
		$sql_search .= "";
	}

	if($category){
		$sql_search .= " AND ( category_name = '{$category}' or category_name = '' )";
	}
...
function get_skin_stylesheet($skin_path, $dir='')
{
    ...
    $skin_url = MS_URL.str_replace("\\", "/", str_replace(MS_PATH, "", $skin_path));
...
class html_process {
    ...
    function run()
    {
        ...
        if($tmp_row['cnt']) {
            $tmp_sql = " update shop_login set mb_id = '{$member['id']}', lo_datetime = '".MS_TIME_YMDHIS."', lo_location = '{$ms['lo_location']}', lo_url = '{$ms['lo_url']}' where lo_ip = '{$_SERVER['REMOTE_ADDR']}' ";
            sql_query($tmp_sql, FALSE);
        } else {
            $tmp_sql = " insert into shop_login ( lo_ip, mb_id, lo_datetime, lo_location, lo_url ) values ( '{$_SERVER['REMOTE_ADDR']}', '{$member['id']}', '".MS_TIME_YMDHIS."', '{$ms['lo_location']}',  '{$ms['lo_url']}' ) ";
            sql_query($tmp_sql, FALSE);

            // 시간이 지난 접속은 삭제한다
            sql_query(" delete from shop_login where lo_datetime < '".date("Y-m-d H:i:s", MS_SERVER_TIME - (60 * 10))."' ");

            // 부담(overhead)이 있다면 테이블 최적화
            //$row = sql_fetch(" SHOW TABLE STATUS FROM `$mysql_db` LIKE '$ms['login_table']' ");
            //if($row['Data_free'] > 0) sql_query(" OPTIMIZE TABLE $ms['login_table'] ");
        }
        ...
        if(!empty($links)) {
            ...
            foreach($links as $link) {
                ...
                $link[1] = preg_replace('#\.css([\'\"]?>)$#i', '.css?ver='.MS_CSS_VER.'$1', $link[1]);
        ...
        if(!empty($scripts)) {
            ...
            foreach($scripts as $js) {
                ...
                $js[1] = preg_replace('#\.js([\'\"]?>)$#i', '.js?ver='.MS_JS_VER.'$1', $js[1]);
...
// 연속배너 sql
function sql_banner_rows($code, $mb_id, $mb_grade ='', $mb_category = '')
{
	global $config, $mk;

	if(MS_IS_MOBILE) // 모바일접속인가?
	...
	if($mb_grade){
		$sql_where .= " and mb_grade IN (0, {$mb_grade})  ";
	}
	//echo "mb_category:".$mb_category;
	if($mb_category){
		$sql_where .= " and category_name = '{$mb_category}' ";
	}
	...
	$row = sql_fetch($sql);
	if(!$row['bn_id'] && $mb_id != encrypted_admin()) {
		$sql = " select * from shop_banner {$sql_where} and mb_id = '".encrypted_admin()."' {$sql_order} ";
	}
...
function check_banner_copy($mb_id, $device, $theme)
{
	...
	for($i=0; $cp=sql_fetch_array($result); $i++)
	{
		...
		foreach($fields as $fld) {
			if(in_array($fld, array('bn_id', 'mb_id', 'bn_file', 'bn_theme'))) continue;

			$sql_common .= " , $fld = '".addslashes($cp[$fld])."' ";
		}

		$sql = " insert into shop_banner set mb_id = '$mb_id' , bn_theme = '$theme' $sql_common ";
		sql_query($sql);
...
function get_skin_path($dir, $skin)
{
    ...
    if(preg_match('#^theme/(.+)$#', $skin, $match)) { // 테마에 포함된 스킨이라면
        ...
        if(G5_IS_MOBILE) {
            $skin_path = $theme_path.'/'.G5_SKIN_DIR.'/'.$match[1];
            if(!is_dir($skin_path))
                $skin_path = $theme_path.'/'.G5_SKIN_DIR.'/'.$match[1];
        } else {
            $skin_path = $theme_path.'/'.G5_SKIN_DIR.'/'.$match[1];
