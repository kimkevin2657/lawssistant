...
if($is_seometa == 'it') { // 상품
	...
	$seometa['url'] = MS_SHOP_URL.'/view.php?index_no='.$gs['index_no'];
...
$seometa['title'] = ($seometa['title']) ? $seometa['title'] : $ms['title'];
...
$seometa['url'] = ($seometa['url']) ? $seometa['url'] : MS_URL;
...
if($seometa['img'] && preg_match("/(\.(jpg|jpeg|gif|png))$/i", $seometa['img'])) {
	$seometa['img'] = $seometa['img'].'?v='.MS_TIME_YHS;
}
?>
