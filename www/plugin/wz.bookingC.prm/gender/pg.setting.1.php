...
if ($wzpconfig['pn_pg_service'] == 'inicis') { 
    ...
    $log_path = WZB_PLUGIN_PATH.'/gender/inicis/log';

    if(!is_dir($log_path)) {
        echo '<script>'.PHP_EOL;
        echo 'alert("'.str_replace(TB_PATH.'/', '', WZB_PLUGIN_PATH).'/gender/inicis 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
        echo '</script>'.PHP_EOL;
    } else {
...
else if ($wzpconfig['pn_pg_service'] == 'lg') { 
    
    $log_path = WZB_PLUGIN_PATH.'/gender/lg/lgxpay/lgdacom/log';

    if(!is_dir($log_path)) {
        echo '<script>'.PHP_EOL;
        echo 'alert("'.str_replace(TB_PATH.'/', '', WZB_PLUGIN_PATH).'/gender/lg/lgxpay/lgdacom/log 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
        echo '</script>'.PHP_EOL;
    } else {
        if(!is_writable($log_path)) {
            echo '<script>'.PHP_EOL;
...
else if ($wzpconfig['pn_pg_service'] == 'kcp') { 
    ...
    //echo module_exec_check(WZB_PLUGIN_PATH.'/gender'.$exe, 'pp_cli');

    // log 디렉토리 체크 후 있으면 경고
    if(is_dir(WZB_PLUGIN_PATH.'/gender/kcp/log') && is_writable(WZB_PLUGIN_PATH.'/gender/kcp/log')) {
        echo '<script>'.PHP_EOL;
...
// 이니시스 웹결제 사인키 필드 추가.
$query = "show columns from `{$g5['wzb_config_table']}` like 'pn_pg_sign_key' ";
$res = sql_fetch($query);
if (empty($res)) {
    sql_query(" ALTER TABLE `{$g5['wzb_config_table']}` 
                    ADD `pn_pg_sign_key` varchar(255) NOT NULL
                    ; ", true);
}

// 모바일 이니시스 계좌이체 결과 전달을 위한 테이블 추가
if(!sql_query(" DESCRIBE g5_wzb_inicis_log ", false)) {
    sql_query(" CREATE TABLE IF NOT EXISTS `g5_wzb_inicis_log` (
...
<tr>
    ...
    <td>
        ...
        <select id="pn_pg_service" name="pn_pg_service">
            ...
            <option value="inicis" <?php echo get_selected($wzpconfig['pn_pg_service'], 'inicis'); ?>>KG이니시스</option>
            <option value="kcp" <?php echo get_selected($wzpconfig['pn_pg_service'], 'kcp'); ?>>KCP</option>
            <option value="lg" <?php echo get_selected($wzpconfig['pn_pg_service'], 'lg'); ?>>LG유플러스</option>
        </select>
    </td>
</tr>
<tr>
    <th scope="row"><label for="pn_pg_mid" id="pn-pg-title01">상점아이디</label></th>
    <td>
...
<tr>
    <th scope="row"><label for="pn_pg_site_key" id="pn-pg-title02">상점키코드</label></th>
    <td>
...
<tr>
    <th scope="row"><label for="pn_pg_sign_key" id="pn-pg-title03">상점부가코드</label></th>
    <td>
        <div id="pn-pg-desc03"><?php echo help("결제대행사에서 발급받은 부가적인 코드정보를 입력해주세요."); ?></div>
        <input type="text" name="pn_pg_sign_key" value="<?php echo $wzpconfig['pn_pg_sign_key']; ?>" id="pn_pg_sign_key" class="frm_input" size="70" maxlength="110">
    </td>
</tr>
<tr>
...
<tr>
    <th scope="row"><label for="pn_pg_dbank_use">계좌이체사용</label></th>
    <td>
        <?php echo help("주문시 계좌이체 결제를 가능하게 할것인지를 설정합니다.", 50); ?>
        <select id="pn_pg_dbank_use" name="pn_pg_dbank_use">
            <option value="0" <?php echo get_selected($wzpconfig['pn_pg_dbank_use'], 0); ?>>사용안함</option>
            <option value="1" <?php echo get_selected($wzpconfig['pn_pg_dbank_use'], 1); ?>>사용</option>
        </select>
    </td>
</tr>
<tr>
    <th scope="row"><label for="pn_pg_vbank_use">가상계좌사용</label></th>
    <td>
        <?php echo help("주문시 가상계좌 결제를 가능하게 할것인지를 설정합니다.", 50); ?>
        <select id="pn_pg_vbank_use" name="pn_pg_vbank_use">
            <option value="0" <?php echo get_selected($wzpconfig['pn_pg_vbank_use'], 0); ?>>사용안함</option>
            <option value="1" <?php echo get_selected($wzpconfig['pn_pg_vbank_use'], 1); ?>>사용</option>
        </select>

        <?php if ($wzpconfig['pn_pg_service'] == 'inicis') { ?>
        <div style="margin:7px 0 0">
            ...
            <?php echo WZB_PLUGIN_URL; ?>/gender/inicis/pg_vbank_sink.php
        </div>
        <?php } ?>
...
<tr>
    <th scope="row"><label for="pn_pg_hp_use">휴대폰결제사용</label></th>
    <td>
        <?php echo help("주문시 가상계좌 결제를 가능하게 할것인지를 설정합니다.", 50); ?>
        <select id="pn_pg_hp_use" name="pn_pg_hp_use">
            <option value="0" <?php echo get_selected($wzpconfig['pn_pg_hp_use'], 0); ?>>사용안함</option>
            <option value="1" <?php echo get_selected($wzpconfig['pn_pg_hp_use'], 1); ?>>사용</option>
        </select>
    </td>
</tr>
...
jQuery(document).ready(function () {
    $(document).on("change", "#pn_pg_service", function() {
        set_pg_input();
    });
    set_pg_input();
});
function set_pg_input() {
    var pg_service = $("#pn_pg_service > option:selected").val();
    if (pg_service == 'etc') {
