...
<?php
    ...
    if ( $tx_cd == "TX00" )
    {
        $query = " select bk_ix, od_id, bk_misu from {$g5['wzb_booking_table']} where od_id = '$order_no' and bk_tno = '$tno' and bk_status != '완료' ";
        $bk = sql_fetch($query);

        if($bk['od_id']) {

            $bk_receipt_price   = (int)$ipgm_mnyx;
            $bk_misu            = $bk['bk_misu'] - $bk_receipt_price;
            $bk_receipt_time    = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})/", "\\1-\\2-\\3 \\4:\\5:\\6", $tx_tm);
