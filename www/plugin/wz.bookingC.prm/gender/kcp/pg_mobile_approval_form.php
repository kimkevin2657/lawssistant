...
<?php
    ...
    /* = -------------------------------------------------------------------------- = */
     ...
     include_once(WZB_PLUGIN_PATH.'/gender/kcp/config.php'); // 환경설정 파일 include
?>
<?php
    /* = -------------------------------------------------------------------------- = */
...
<script src="<?php echo WZB_PLUGIN_URL; ?>/gender/kcp/pg_mobile_approval_key.js"></script>
...
if($enc_data != '' && $enc_info != '' && $tran_cd != '') {
    ...
    $sql = " select * from {$g5['wzb_booking_data_table']} where od_id = '$ordr_idxx' ";
    $row = sql_fetch($sql);
...
<input type="hidden" name="Ret_URL"      value="<?php echo WZB_PLUGIN_URL; ?>/gender/kcp/pg_mobile_approval_form.php">
<!-- 인증시 필요한 파라미터(변경불가)-->
...
<input type="hidden" name="enc_data"       value="<?php echo $enc_data; ?>">    <!-- 암호화 데이터      -->
    ...
    <div id="show_progress" style="display:none;">
        <span style="display:block; text-align:center;margin-top:120px"><img src="<?php echo WZB_PLUGIN_URL; ?>/gender/kcp/img/loading.gif" alt="" ></span>
