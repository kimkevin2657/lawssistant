...
require_once(WZB_PLUGIN_PATH.'/gender/inicis/libs/HttpClient.php');
require_once(WZB_PLUGIN_PATH.'/gender/inicis/libs/json_lib.php');
...
try {
    ...
    //alert("성공전");
    //alert("code:".$_REQUEST['resultCode']);
    //alert("request:".$_REQUEST);
    if ($_REQUEST['resultCode'] == '0000' || $_REQUEST['resultCode'] == 0000 ) {
    //if (strcmp('0000', $_REQUEST['resultCode']) == 0) {
        
        //alert("resultCode:".$_REQUEST['resultCode']);
        ...
        try {
            ...
            $sql = " select * from {$g5['wzb_booking_data_table']} where od_id = '$oid' ";
            $row = sql_fetch($sql);

            $data = unserialize(base64_decode($row['dt_data']));

            $order_action_url = https_url(G5_PLUGIN_DIR.'/wz.bookingC.prm/step.2.update.php', true);
            $page_return_url  = WZB_STATUS_URL;


            if ((strcmp('0000', $resultMap['resultCode']) == 0) && (strcmp($secureSignature, $resultMap['authSignature']) == 0) ) { //결제보안 추가

                //alert("추가보안 성공:".$resultMap['resultCode']);
                /*                         * ***************************************************************************
                ...
                require WZB_PLUGIN_PATH.'/gender/inicis/INIStdPayResult.php';
                exit;
            } else {
