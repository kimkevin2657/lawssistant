...
function pg_pay(f) {
    ...
    <?php } else { ?>
        ...
        pf.good_mny.value = pf.reserv_price.value;
...
function make_signature(frm)
{
    ...
    var result = true;
    $.ajax({
        url: "<?php echo WZB_PLUGIN_URL?>/gender/inicis/makesignature.php",
        type: "POST",
        data: {
            bo_table : "<? echo $_GET['bo_table']; ?>",
...
<?php if ($is_mobile_pay) { // 모바일 환경일경우 ?>
    ...
    <input type="hidden" name="P_AMT"        value="<?php echo $reserv_price; ?>">
    <input type="hidden" name="P_UNAME"      value="">
    <input type="hidden" name="P_MOBILE"     value="">
    <input type="hidden" name="P_EMAIL"      value="">
    <input type="hidden" name="P_MID"        value="<?php echo $wzpconfig['pn_pg_mid']; ?>">
