...
include_once(WPOT_PLUGIN_PATH.'/lib/core.lib.php');
...
if($P_TYPE == "VBANK")	//결제수단이 가상계좌이며
{
   ...
   // 입금결과 처리
    $query = " select bk_ix, od_id from {$g5['wpot_order_table']} where od_id = '$P_OID' and bk_tno = '$P_TID' and bk_status != '완료' ";
    $bk = sql_fetch($query);
    ...
    if($bk['od_id']) {
        ...
        $query = " update {$g5['wpot_order_table']}
...
if($P_TYPE == 'BANK' || $P_SRC_CODE == 'A') {
    $sql = " insert into {$g5['wpot_order_inicis_log_table']}
                set oid       = '$P_OID',
                    P_TID     = '$P_TID',
                    P_MID     = '$P_MID',
                    P_AUTH_DT = '$P_AUTH_DT',
