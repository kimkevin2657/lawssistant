...
if($write['issecret'] == 'Y') {
	...
	if($is_member) {
		...
		if($_GET['inpasswd'] != $write['passwd']) {
			goto_url(MS_MBBS_URL."/board_secret.php?index_no=$index_no&boardid=$boardid&page=$page");
		}
	}
}
...
if($boardid=="102") {
	Theme::get_theme_part(MS_MTHEME_PATH,'/board_read_property.skin.php');
}else{
	Theme::get_theme_part(MS_MTHEME_PATH,'/board_read.skin.php');
}
