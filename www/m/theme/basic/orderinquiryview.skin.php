...
<div id="sod_fin">
	...
	<section id="sod_fin_list">
        ...
        <ul id="sod_list_inq" class="sod_list">
			...
			$totalcountc = sql_num_rows($result);
			for($i=0; $row=sql_fetch_array($result); $i++) {
				...
				$li_btn = '';
				if($rw['dan'] == 5) {
					$rd = get_review($od['od_id']);
								if(!$rd['od_id']) {
					$li_btn .= '<a href="'.MS_MSHOP_URL.'/orderreview.php?gs_id='.$rw['gs_id'].'" onclick="win_open(this, \'winorderreview\');return false;" class="btn_ssmall bx-white">구매후기</a>';
				}}
			...
			<li class="sod_li">
                ...
                <div class="li_prqty">
                    ...
                    <span class="prqty_stat li_prqty_sp"><span>상태 </span><?php echo $gw_status[$rw['dan']]; ?>
				<?php if($rw['dan'] == '2' and $rw['paymethod'] == "신용카드"){
						if($totalcountc == '1'){ ?>
						&nbsp;<a href="#" onclick="fcancel_check2();" class="btn_small">주문취소</a>
					<?php }else{ ?>
						&nbsp;<a href="<?php echo MS_MSHOP_URL; ?>/orderinquirycancel_bubun.php?od_no=<?php echo $row['od_no']; ?>" onclick="return confirm('주문취소를 하시겠습니까 ?');" class="btn_small">주문취소</a>
					<?php  }?>
				<?php } ?>
					</span>
                </div>
		...
		<dl id="sod_bsk_tot">
            ...
            <dt class="sod_bsk_dvr"><span>쇼핑페이결제</span></dt>
            <dd class="sod_bsk_dvr"><strong><?php echo display_point($stotal['usesppoint']); ?></strong></dd>
            <?php } ?>
            ...
            <dt class="sod_bsk_point"><span>마일리지적립</span></dt>
            <dd class="sod_bsk_point"><strong><?php echo str_replace("P","원",display_point($stotal['kv_point'])); ?></strong></dd>
            <dt class="sod_bsk_point"><span>쇼핑포인트적립</span></dt>
            <dd class="sod_bsk_point"><strong><?php echo display_point($stotal['point']); ?></strong></dd>


            <!-- <dt class="sod_bsk_point"><span>쇼핑페이적립</span></dt>
            <dd class="sod_bsk_point"><strong><?php echo display_point($stotal['sp_point']); ?></strong></dd> -->
        </dl>
    </section>
	...
	<section id="sod_fin_pay">
		...
		<div class="odf_tbl">
			...
			<tr>
				...
				?>
					<a href="javascript:;" onclick="window.open('<?php echo MS_MSHOP_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');" class="btn_small">현금영수증 발급하기</a>
				<?php } ?>
				</td>
			</tr>
			<?php
	...
	<section id="sod_fin_cancel">
		...
		<div id="sod_fin_cancelfrm">
			<form method="post" action="<?php echo MS_MSHOP_URL; ?>/orderinquirycancel.php" name="chi" onsubmit="return fcancel_check(this);">
