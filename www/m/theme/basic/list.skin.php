...
<?php
$cgy = m_get_category_head_image($ca_id);
echo $cgy['img_head']; // 분류별 상단이미지 

echo mobile_tree_category($ca_id); // 하위분류
?>
...
.naverrp{position: absolute;top:0;float:right;left:5px;z-index: 998;width:40px;margin-top:5px;}
</style-->
	...
	if(!$total_count) {
      ...
      $kgb = $i + 1;
			...
			if($row['dongurl']){
				...
				echo "<dt><img src=\"{$it_imageurl}\">";
                	if($bestgood=="Y") { 
			if($i < 50){
			echo "<span class=\"bnumber\">BEST ".$kgb."</span>";
          }}
