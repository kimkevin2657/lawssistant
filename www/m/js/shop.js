...
$(function() {
    ...
    if(isAndroid) {
        $("select.it_option").live("touchstart", function() {
            option_add = true;
        });
    } else {
...
function add_sel_option(type, id, option, price, stock, amt)
{
    ...
    opt += "<input type=\"hidden\" class=\"io_stock\" value=\""+stock+"\">\n";
		opt += "<ul>\n";
			...
			opt += "<li class=\"it_qty\">\n";
				opt += "<dl class=\"fl\">\n";
					opt += "<dt class=\"fl padr3\"><button type=\"button\" class=\"btn_small grey\">-</button></dt>\n";
...
function price_calculate()
{
	...
	if(document.getElementById('it_price2')){
		var it_price2 = parseInt($("input#it_price2").val());
	}else{
		var it_price2 = 0;
	}
    ...
    var price, type, qty, total = 0, total2 = 0;
    ...
    $el_prc.each(function(index) {
        ...
        if(type == "0") { // 주문옵션
            ...
            total2 += (it_price-it_price2 + price) * qty;
        } else { // 추가옵션
            total += price * qty;
            total2 += price * qty;
        }
	...
	if(document.getElementById('sit_tot_views2')){
		$("#sit_tot_views2").show();
		$("#sit_tot_price2").empty().html(number_format(String(total2))+"원");
	}
}
