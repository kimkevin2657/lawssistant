...
$subject_1 = '배송중';
$message_1 = $od['name'].'님께 '.$item['gname'].'이 발송되었습니다.

배송업체 : '.$item['delivery'].'

송장번호 : '.number_format($item['delivery_no']).'

이용해주셔서 감사합니다.';

aligo_sms('TE_4899', $od['cellphone'], $od['name'], $subject_1, $message_1);
...
$subject_1 = '배송완료';
$message_1 = $od['name'].'님께서 주문하신 '.$item['gname'].'이
배송 완료되었습니다.
...
aligo_sms('TE_4900', $od['cellphone'], $od['name'], $subject_1, $message_1);
...
if($od_sms_ipgum_check)
	icode_order_sms_send($od_hp, 3, $od_id); // 입금완료 문자

if($od_sms_baesong_check)
	icode_order_sms_send($od_hp, 4, $od_id); // 배송중 문자

if($od_sms_delivered_check)
	icode_order_sms_send($od_hp, 6, $od_id); // 배송완료 문자

if($od_sms_cancel_check)
	icode_order_sms_send($od_hp, 5, $od_id); // 주문취소 문자
//------------------------------------------------------------------------------
...
if($od_cancel_change) {
	...
	if($row['od_count1'] == $row['od_count2']) {
			...
			if($od['od_tno'] && ($od['paymethod'] == '신용카드' || $od['paymethod'] == '간편결제' || $od['paymethod'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && $od['paymethod'] == '삼성페이')) {
				...
				switch($od['od_pg']) {
                    case 'easypay':
						...
						{
							$easyPay->set_easypay_deli_us( $mgr_data, "mgr_txtype"      , "40"   );
							$easyPay->set_easypay_deli_us( $mgr_data, "mgr_subtype"     , "RF01" );
						}
						$easyPay->set_easypay_deli_us( $mgr_data, "org_cno"         , $od['od_tno']     );
						$easyPay->set_easypay_deli_us( $mgr_data, "order_no"        , $od['od_no']  );
						$easyPay->set_easypay_deli_us( $mgr_data, "req_ip"          , $client_ip );
						$easyPay->set_easypay_deli_us( $mgr_data, "req_id"          , $od['mb_id'] );
						$easyPay->set_easypay_deli_us( $mgr_data, "mgr_msg"         , $cancel_memo  );
						$opt = "option value";
						$easyPay->easypay_exec($default['de_easypay_mid'], $tr_cd,  $od['od_no'], $client_ip, $opt);
						$res_cd   = $easyPay->_easypay_resdata["res_cd"     ];    // 응답코드
						$res_msg  = iconv_utf8($easyPay->_easypay_resdata["res_msg"    ]);    // 응답메시지
                        ...
                        break;
					case 'lg':
						include_once(MS_SHOP_PATH.'/settle_lg.inc.php');
						...
						break;
					case 'inicis':
						include_once(MS_SHOP_PATH.'/settle_inicis.inc.php');
						...
						break;
					case 'KAKAOPAY':
						include_once(MS_SHOP_PATH.'/settle_kakaopay.inc.php');
						$_REQUEST['TID']               = $od['od_tno'];
						$_REQUEST['Amt']               = $od_receipt_price;
						$_REQUEST['CancelMsg']         = '쇼핑몰 운영자 승인 취소';
						$_REQUEST['PartialCancelCode'] = 0;
						include MS_SHOP_PATH.'/kakaopay/kakaopay_cancel.php';
						break;
					case 'kcp':
						include_once(MS_SHOP_PATH.'/settle_kcp.inc.php');
						require_once(MS_SHOP_PATH.'/kcp/pp_ax_hub_lib.php');
		...
		// 관리자 주문취소 로그
		$mod_history = MS_TIME_YMDHIS.' '.$member['id'].' 주문취소 처리'.$pg_cancel_log."\n";
    }
}
...
$url = MS_ADMIN_URL."/pop_orderform.php?od_id=$od_id";
