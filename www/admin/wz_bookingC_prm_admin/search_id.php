...
$sql_search = " where grade IN (2, 5) ";

if($stx){
    $sql_search .= " and ( id like '%{$stx}%' or name like '%{$stx}%') ";
}
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <td><? echo $row['id']; ?></td>
        <td><? echo $row['name']; ?></td>
        <td><span class="sel_id" store_mb_id="<? echo $row['id']; ?>" store_name="<? echo $row['name']; ?>">선택</span></td>
    </tr>
    <? } ?>
    </tbody>
    </table>
...
<script>
    $(".sel_id").on("click", function(){
        store_mb_id = $(this).attr("store_mb_id");
        store_name = $(this).attr("store_name");
        $("#store_mb_id", opener.document).val(store_mb_id);
        $(".store_name", opener.document).text(store_name);
	self.close();
    })
</script>
