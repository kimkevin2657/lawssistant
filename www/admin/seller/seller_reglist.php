...
//if( $target != 'up_id') $sql_search.= " and a.grade <> '1' and a.supply='' and b.mb_id is null "
if($sfl) {
...
?>
            ...
            <table>
                ...
                for($i=0; $row=sql_fetch_array($result); $i++) {
                    ...
                    <tr class="<?php echo $bg; ?>">
                        ...
                        <td><button type="button" onClick="yes('<?php echo $row['id']; ?>', '<?php echo $row['id']; ?>', '<?php echo $row['index_no']; ?>', '<?php echo $row['name']; ?>', '<?php echo $row['grade']?>')" class="btn_small grey">선택</button></td>
                    </tr>
                    <?php
                }
                if($i==0)
    ...
    <script>
        function yes(mb_id, plain_id, index_no, name, grade){
            if( opener.setUser ) {
                opener.setUser({
                    id : mb_id,
                    plain_id : plain_id,
            ...
            } else if( "<?php echo $target; ?>" != "" && opener.document.fregform ){
                opener.document.fregform.<?php echo $target; ?>.value = mb_id;
            } else {
                opener.document.setUser({
                    id : mb_id,
                    plain_id : plain_id,
                    no : index_no,
                    nm : name,
