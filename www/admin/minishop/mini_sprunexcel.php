...
$excel->setActiveSheetIndex(0)
	...
	->setCellValue($char++.'1', '현재잔액P')
	->setCellValue($char++.'1', '출금요청P')
	->setCellValue($char++.'1', '세액공제P')
	->setCellValue($char++.'1', '실수령액P')
...
for($i=2; $row=sql_fetch_array($result); $i++)
{
	...
	$excel->setActiveSheetIndex(0)
		...
		->setCellValueExplicit($char++.$i, $row['sp_point'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
...
// Rename worksheet
$excel->getActiveSheet()->setTitle('가맹점 쇼핑페이 환전');
...
header('Content-Disposition: attachment; filename="가맹점 쇼핑페이 환전-'.date("ymd", time()).'.xlsx"');
