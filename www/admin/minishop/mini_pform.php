...
<form name="pform" id="pform" method="post" action="./minishop/mini_pformupdate.php">
...
<section id="anc_pbasic">
	...
	<div class="tbl_frm01">
		...
		<tr>
			<td class="bo_label">
				<label><input type="radio" name="pf_sale_flag" value="0"<?php echo get_checked($config['pf_sale_flag'], "0"); ?>> <strong class="fc_red">결제액 - 배송비 - 쿠폰 - 쇼핑페이 - 쇼핑포인트결제액 = 순수결제액</strong> 에서 판매수수료를 배분</label><br>
				<label><input type="radio" name="pf_sale_flag" value="1"<?php echo get_checked($config['pf_sale_flag'], "1"); ?>> <strong class="fc_red">판매가 - 공급가 - 쿠폰 - 쇼핑페이 - 쇼핑포인트결제액 = 마진</strong> 에서 판매수수료를 배분<span>(마진이 없으면 적립되지 않음)</span></label><br>
				<label><input type="radio" name="pf_sale_flag" value="2"<?php echo get_checked($config['pf_sale_flag'], "2"); ?>> <strong class="fc_red">판매가 - 공급가 = 마진</strong> 에서 판매수수료를 배분<span>(쿠폰 및 쇼핑포인트 사용액은 무시하고 무조건 적립)</span></label><br>
               <label><input type="radio" name="pf_sale_flag" value="3"<?php echo get_checked($config['pf_sale_flag'], "3"); ?>> <strong class="fc_red">마일리지</strong></label> 
			</td>
		</tr>
		<tr>
		<th scope="row">분양 후원수수료</th>
			<td>
				...
				<?php echo help('기존 가맹점의 후원으로 다시 쇼핑몰이 분양될경우 후원수수료를 지급합니다.'); ?>
			</td>
		</tr>
		<tr>
...
<section id="anc_panew">
	...
	<div class="tbl_frm01">
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="minishop_reg_yes" value="1" id="minishop_reg_yes" 
				<?php echo get_checked($config['minishop_reg_yes'], "1"); ?>>
				<label for="minishop_reg_yes">사용함</label>
...
<section id="anc_pauth">
	...
	<div class="tbl_frm01">
		...
		<tr>
			<th scope="row"><label for="pf_auth_cgy">카테고리 권한</label></th>
			<td>
				<select name="pf_auth_cgy" id="pf_auth_cgy">
					<?php echo option_selected("1", $config['pf_auth_cgy'], '본사 카테고리 고정'); ?>
					<?php echo option_selected("2", $config['pf_auth_cgy'], '개별 카테고리 허용'); ?>
				</select>
				<?php echo help('<em class="fc_084"><i class="fa fa-exclamation-circle fs11 fc_084"></i> 본사 카테고리 고정 : </em>본사 카테고리를 수정은 할수있지만 개별적으로 생성은 불가합니다.<br><em class="fc_084"><i class="fa fa-exclamation-circle fs11 fc_084"></i> 개별 카테고리 허용 : </em>본사 카테고리 + 개별 카테고리 혼합설정 가능합니다.', 'lh5'); ?>				
			</td>
		</tr>
		<tr>
