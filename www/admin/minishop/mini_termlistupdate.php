...
if($_POST['act_button'] == "선택승인")
{
	for($i=0; $i<$count; $i++)
	{
		...
		$row = get_minishop_term($index_no);
		...
		sql_query("update shop_minishop_term set state = '1' where index_no = '$index_no'");	
	}
} 
else if($_POST['act_button'] == "선택삭제") 
{
	for($i=0; $i<$count; $i++)
	{
		...
		sql_query("delete from shop_minishop_term where index_no = '$index_no'");
	}
}
