...
<div class="tbl_head01">
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td><?php echo get_delivery_inquiry($row2['delivery'], $row2['delivery_no'], 'btn_ssmall'); ?>
        </td>
		<?php if($k == 0) { ?>
...
(function($){
    $(function () {
       $('.delivery-search-btn').on('click', function(e){
           e.preventDefault();
           var invoiceNo = $(this).data('invoice');
           var invoiceCo = $(this).closest('tr').find('td>select.delivery-company').val();
           var arrCompInf= invoiceCo.split("|");
           if( arrCompInf.length > 1) window.open(arrCompInf[1]+invoiceNo);
       });
    })
}(jQuery));
</script>
