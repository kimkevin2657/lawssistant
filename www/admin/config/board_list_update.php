...
else if($_POST['act_button'] == "선택삭제") 
{
	...
	for($i=0; $i<$count; $i++)
	{
		...
		while($board=sql_fetch_array($tmpData)) {
			if($board['fileurl1']) @unlink(MS_DATA_PATH."/board/".$bo_table."/".$board['fileurl1']);
			if($board['fileurl2']) @unlink(MS_DATA_PATH."/board/".$bo_table."/".$board['fileurl2']);
			delete_editor_image($board['memo']);
		}

		if(is_dir(MS_DATA_PATH."/board/".$bo_table)) 
			zRmDir(MS_DATA_PATH."/board/".$bo_table);
...
goto_url(MS_ADMIN_URL."/config.php?$q1&page=$page");
?>