...
$upl_dir = MS_DATA_PATH."/board/boardimg";
...
if($w == "") {
	...
	$value['mb_category']			= $_POST['mb_category'];
	...
	include_once(MS_ADMIN_PATH."/config/board_sql.php");
	@sql_query($board_table);
	@sql_query($board_tail_table);
	$path = MS_DATA_PATH."/board/".$bo_table;
	exec("mkdir  $path");
 	exec("chmod 707 $path");

	goto_url(MS_ADMIN_URL."/config.php?code=board_form&w=u&bo_table=$bo_table");
	...
	//echo "content: ".$_POST['insert_content'];

	update("shop_board_conf",$value," where index_no='$bo_table'");
	
	goto_url(MS_ADMIN_URL."/config.php?code=board_form&w=u&bo_table=$bo_table$qstr&page=$page");
}
?>