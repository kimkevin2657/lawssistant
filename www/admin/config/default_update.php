...
update("shop_member",$value,"where id='{$encrypted_admin}'");
...
$value['de_bae_keep_term']	= $_POST['de_bae_keep_term']; //배송중 자동 배송완료
update("shop_default", $value);

$common_dir = MS_DATA_PATH."/common";
...
$value['login_point']			= conv_number($_POST['login_point']); //로그인 쇼핑포인트
$value['usepoint']				= conv_number($_POST['usepoint']); //구매시쇼핑포인트
$value['usepoint_yes']			= $_POST['usepoint_yes']; //쇼핑포인트결제 사용
...
$value['cert_minishop_yes']		= $_POST['cert_minishop_yes']; //회원 승인권한
...
$value['basic_goods_kv'] = conv_number($_POST['basic_goods_kv']); // 기본 마일리지 원
$value['basic_goods_kv_per'] = conv_number($_POST['basic_goods_kv_per']); // 기본 마일리지 %
$value['basic_gpoint'] = conv_number($_POST['basic_gpoint']); // 기본 쇼핑포인트 P
$value['basic_gpoint_per'] = conv_number($_POST['basic_gpoint_per']); // 기본 쇼핑포인트 %
...
$brand_dir  = MS_DATA_PATH."/brand";
$order_dir	= MS_DATA_PATH."/order";

// 회원초기화
if(isset($_POST['clear_1']) && $_POST['clear_1']) {

	// 회원 로고
	$sql = "select * from shop_logo where mb_id!='{$encrypted_admin}' ";
	$res = sql_query($sql);
	...
	sql_query("delete from shop_logo where mb_id!='{$encrypted_admin}' ");

	// 회원 배너
	$sql = "select * from shop_banner where mb_id!='{$encrypted_admin}' ";
	$res = sql_query($sql);
	for($i=0; $row=sql_fetch_array($res); $i++) {
		if($row['bn_file']) { @unlink($banner_dir."/".$row['bn_file']); }
	}
	sql_query("delete from shop_banner where mb_id!='{$encrypted_admin}' ");
	...
	sql_query("delete from shop_member where id !='{$encrypted_admin}'"); //회원정보 내역
	...
	sql_query("delete from shop_minishop");
	sql_query("ALTER TABLE shop_minishop AUTO_INCREMENT=1");
	sql_query("delete from shop_minishop_pay"); //가맹점 수수료 적립내역
	sql_query("ALTER TABLE shop_minishop_pay AUTO_INCREMENT=1");
	sql_query("delete from shop_minishop_payrun"); //실시간 출금요청
	sql_query("ALTER TABLE shop_minishop_payrun AUTO_INCREMENT=1");
	sql_query("delete from shop_minishop_term"); //관리비연장 신청내역
	sql_query("ALTER TABLE shop_minishop_term AUTO_INCREMENT=1");
	sql_query("delete from shop_point"); //쇼핑포인트내역
	...
	// 상품정보
	$sql = "select * from shop_goods where mb_id!='".encrypted_admin()."' ";
	$res = sql_query($sql);
	for($i=0; $row=sql_fetch_array($res); $i++) {
	...
	sql_query("delete from shop_goods where mb_id!='{$encrypted_admin}'");
	sql_query("delete from shop_goods_type where mb_id!='{$encrypted_admin}'");

	// 팝업삭제
	$sql = "select * from shop_popup where mb_id!='{$encrypted_admin}' ";
	$res = sql_query($sql);
	for($i=0; $row=sql_fetch_array($res); $i++) {
		delete_editor_image($row['memo']);
	}
	sql_query("delete from shop_popup where mb_id!='{$encrypted_admin}'");

	sql_query("delete from shop_goods_qa");// 상품문의 테이블
	sql_query("ALTER TABLE shop_goods_qa AUTO_INCREMENT=1");
	sql_query("delete from shop_brand where mb_id!='{$encrypted_admin}'"); // 브랜드정보
	sql_query("delete from shop_keyword where pt_id!='{$encrypted_admin}'"); // 검색 키워드
	sql_query("delete from shop_visit where mb_id!='{$encrypted_admin}'"); // 접속자집계
	sql_query("delete from shop_visit_sum where mb_id!='{$encrypted_admin}'"); // 접속자집계
}
...
if(isset($_POST['clear_2']) && $_POST['clear_2']) {
	...
	rm_rf($order_dir);
	if(!is_dir($order_dir)) {
		@mkdir($order_dir, MS_DIR_PERMISSION);
		@chmod($order_dir, MS_DIR_PERMISSION);
	}
}
...
if(isset($_POST['clear_4']) && $_POST['clear_4']) {
	...
	rm_rf($goods_dir);
	if(!is_dir($goods_dir)) {
		@mkdir($goods_dir, MS_DIR_PERMISSION);
		@chmod($goods_dir, MS_DIR_PERMISSION);
	}
}

// 회원쇼핑포인트 초기화
if(isset($_POST['clear_5']) && $_POST['clear_5']) {
	sql_query("update shop_member set point='0'"); //회원 전체 쇼핑포인트
	sql_query("delete from shop_point"); //쇼핑포인트 내역
	sql_query("ALTER TABLE shop_point AUTO_INCREMENT=1");
}
...
if(isset($_POST['clear_8']) && $_POST['clear_8']) {
	...
	rm_rf($brand_dir);
	if(!is_dir($brand_dir)) {
		@mkdir($brand_dir, MS_DIR_PERMISSION);
		@chmod($brand_dir, MS_DIR_PERMISSION);
	}
}

goto_url(MS_ADMIN_URL.'/config.php?code=default');
?>