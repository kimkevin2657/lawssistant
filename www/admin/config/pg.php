	...
	<tr>
		...
		<td>
			<select name="de_pg_service" id="de_pg_service">
                ...
                <?php echo option_selected("easypay", $default['de_pg_service'], "이지페이"); ?>
				<?php echo option_selected("paywelcome", $default['de_pg_service'], "웰컴페이먼츠"); ?>
			</select>
...
<div class="pg_info_fld easypay_info_fld">
    <h2>이지페이 계약정보</h2>
    <div class="tbl_frm01">
        <table>
            <colgroup>
            ...
            <tbody>
            <tr>
                <th scope="row"><label for="de_easypay_mid">이지페이 상점아이디</label></th>
                <td>
                    <input type="text" name="de_easypay_mid" value="<?php echo $default['de_easypay_mid']; ?>" id="de_easypay_mid" class="frm_input" size="10" maxlength="20">
                     <a href="http://easypay.co.kr/" target="_blank" class="btn_small blue">이지페이 서비스신청하기</a>
                </td>
            </tr>
            </tbody>
        </table>
...
<div class="pg_info_fld paywelcome_info_fld">
	<h2>웰컴페이먼츠 계약정보</h2>
	<div class="tbl_frm01">
		<table>
		<colgroup>
		...
		<tbody>
		<tr>
			<th scope="row"><label for="de_paywelcome_mid">웰컴페이먼츠 상점아이디</label></th>
			<td>
				<input type="text" name="de_paywelcome_mid" value="<?php echo $default['de_paywelcome_mid']; ?>" id="de_paywelcome_mid" class="frm_input" size="10">
				<a href="https://www.welcomepayments.co.kr/" target="_blank" class="btn_small blue">웰컴페이먼츠 서비스신청하기</a>
				<?php echo help('웰컴페이먼츠로 부터 발급 받으신 상점아이디(MID)를 입력해 주십시오.'); ?>
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="de_paywelcome_admin_key">웰컴페이먼츠 키패스워드</label></th>
			<td>
				<input type="text" name="de_paywelcome_admin_key" value="<?php echo $default['de_paywelcome_admin_key']; ?>" id="de_paywelcome_admin_key" class="frm_input" size="5" maxlength="4">
				<?php echo help('웰컴페이먼츠에서 발급받은 4자리 상점 키패스워드를 입력합니다.<br>웰컴페이먼츠 상점관리자 패스워드와 관련이 없습니다.<br>키패스워드 값을 확인하시려면 상점측에 발급된 키파일 안의 readme.txt 파일을 참조해 주십시오.'); ?>
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="de_paywelcome_sign_key">웰컴페이먼츠 웹결제 사인키</label></th>
			<td>
				<input type="text" name="de_paywelcome_sign_key" value="<?php echo $default['de_paywelcome_sign_key']; ?>" id="de_paywelcome_sign_key" class="frm_input" size="40" maxlength="50">
				<?php echo help('웰컴페이먼츠에서 발급받은 웹결제 사인키를 입력합니다.<br>관리자 페이지의 상점정보 > 계약정보 > 부가정보의 웹결제 signkey생성 조회 버튼 클릭, 팝업창에서 생성 버튼 클릭 후 해당 값을 입력합니다.'); ?>
			</td>
		</tr>
		<tr>
			<th scope="row">웰컴페이먼츠 삼성페이 버튼</th>
			<td>
				<input type="checkbox" name="de_paywelcome_samsung_pay_use" value="1" id="de_paywelcome_samsung_pay_use"<?php echo get_checked($default['de_paywelcome_samsung_pay_use'], "1"); ?>>
				<label for="de_samsung_pay_use">사용함</label>
				<?php echo help('주문서 작성 페이지에 웰컴페이먼츠 삼성페이 버튼의 별도 사용 여부를 설정합니다.'); ?>
			</td>
		</tr>
		<tr>
			<th scope="row">가상계좌 입금통보 URL</th>
			<td>
				<?php echo MS_SHOP_URL; ?>/settle_paywelcome_common.php
				<?php echo help('위 주소를 <strong><a href="https://iniweb.paywelcome.com" target="_blank">웰컴페이먼츠 관리자</a> &gt; 거래조회 &gt; 가상계좌 &gt; 입금통보방식선택 &gt; URL 수신 설정</strong>에 입력하셔야 자동으로 입금 통보됩니다.'); ?>
			</td>
		</tr>
		</tbody>
		</table>
...
if($default['de_iche_use'] || $default['de_vbank_use'] || $default['de_hp_use'] || $default['de_card_use']) {
    ...
    // 이지페이 경우 log 디렉토리 체크
    if($default['de_pg_service'] == 'eashypay') {

        $exe = '/easypay80_webpay_php/bin/';
        if($is_linux) {
            if(PHP_INT_MAX == 2147483647) // 32-bit
                $exe .= 'ep_cli';
            else
        ...
        echo module_exec_check(MS_SHOP_PATH.$exe, 'ep_cli');

        $log_path = MS_SHOP_PATH.'/easypay80_webpay_php/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(MS_PATH.'/', '', MS_SHOP_PATH).'/easypay 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
            if(!is_writable($log_path)) {
                echo '<script>'.PHP_EOL;
    ...
    // 웰컴페이먼츠 경우 log 디렉토리 체크
    if($default['de_pg_service'] == 'paywelcome') {
        if(!function_exists('xml_set_element_handler')) {
            echo '<script>'.PHP_EOL;
            echo 'alert("XML 관련 함수를 사용할 수 없습니다.\n서버 관리자에게 문의해 주십시오.");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        ...
        $log_path = MS_SHOP_PATH.'/paywelcome/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(MS_PATH.'/', '', MS_SHOP_PATH).'/paywelcome 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
            if(!is_writable($log_path)) {
                echo '<script>'.PHP_EOL;
