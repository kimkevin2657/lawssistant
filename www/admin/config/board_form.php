...
$board_dir = MS_DATA_PATH."/board/boardimg";
...
<form name="fboardform" method="post" enctype="MULTIPART/FORM-DATA"  onsubmit="return boardform_submit(this);">
...
<div class="tbl_frm02">
	...
	<tr>
		...
		<td>
			...
			<?php if($w=='u') { ?><a href="javascript:location.href='<?php echo MS_ADMIN_URL; ?>/config.php?code=board_list&sfl=gr_id&stx='+document.fboardform.gr_id.value;" class="btn_small grey">동일그룹게시판목록</a><?php } ?>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			...
			if(is_file($fileurl1) && $board['fileurl1']) {
				...
				$fileurl1 = rpc($fileurl1, MS_PATH, MS_URL);
	...
	<tr>
		...
		<td>
			...
			if(is_file($fileurl2) && $board['fileurl2']) {
				...
				$fileurl2 = rpc($fileurl2, MS_PATH, MS_URL);
	...
	<tr>
		...
		<td>
			<?php echo editor_html('insert_content', get_text($board['insert_content'], 0)); ?>
			<!-- <textarea name="insert_content" class="frm_textbox wfull" rows="5"><?php //echo $board['insert_content'] ?></textarea> -->
		</td>
	</tr>
	<tr>
		<th scope="row">레벨</th>
		<td>
			<?php echo get_member_select("mb_grade", $board['mb_grade']); ?>
			<select name="mb_category">
				...
				?>
				<option value="<? echo $row['category_name']; ?>" <? echo $row['category_name'] == $board['mb_category'] ? "selected":""; ?> ><? echo $row['category_name']; ?></option>
				<? } ?>
			</select>
		</td>
	</tr>
...
function boardform_submit(f) {
	<?php echo get_editor_js('insert_content'); ?>
