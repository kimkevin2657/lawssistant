...
if($_POST['act_button'] == "선택삭제")
{
	...
	for($i=0; $i<$count; $i++)
	{
		...
		include(MS_ADMIN_PATH.'/goods/goods_delete.inc.php');
	}
}
...
else if($_POST['act_button'] == "선택상품복사") 
{
	...
	for($i=0; $i<$count; $i++)
	{
		...
		// include 전에 $gs_id 값을 반드시 넘겨야 함
		$gs_id = trim($_POST['gs_id'][$k]);
		include(MS_ADMIN_PATH.'/goods/goods_copy.inc.php');
	}
}
...
else if($_POST['act_button'] == "선택판매가수정") 
{
	for($i=0; $i<$count; $i++)
	{
		// 실제 번호를 넘김
		...
		$sql = "update shop_goods 
				   set goods_price = '{$_POST['goods_price'][$k]}'
				 where index_no = '{$_POST['gs_id'][$k]}'";
		sql_query($sql);
		$sql = "update shop_goods_option 
				   set io_price = '{$_POST['goods_price'][$k]}'
				 where gs_id = '{$_POST['gs_id'][$k]}'";
		sql_query($sql);
	}
	$q1 = str_replace('&crol=1','',$q1);
} 
