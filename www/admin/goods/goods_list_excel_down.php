...
if($excelType_tt){
//echo $excelType_tt;
		$sql_r = " select * from shop_down_excel where muc_code='$excelType_tt' order by sunbun_c asc";
		//echo $sql;
		$result_r = sql_query($sql_r);
		$from = '';
		for($b=0; $rowc=sql_fetch_array($result_r); $b++) {
			$from .= "a.".$rowc['tablecode_c'].",";
		}
...
//echo $excelType_tt;
		...
		//echo $sql;
		$result_r = sql_query($sql_r);
		for($a=0; $rowr=sql_fetch_array($result_r); $a++) {
			$excel->setActiveSheetIndex(0)
			->setCellValue($char++.'1', $rowr['tablename_c']);
		}
}else{
$excel->setActiveSheetIndex(0)
...
for($i=2; $row=sql_fetch_row($result); $i++)
{
	$comma = $gcate = "";
...
if($excelType_tt){
		...
		for($a=0; $rowr=sql_fetch_array($result_r); $a++) {
			$excel->setActiveSheetIndex(0)
			->setCellValueExplicit($char++.$i, $row[$a], PHPExcel_Cell_DataType::TYPE_STRING);
		}
}else{
	$excel->setActiveSheetIndex(0)
