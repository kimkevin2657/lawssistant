...
//    error_reporting(E_ALL ^ E_NOTICE);
	...
	for($i=2; $i<=$data->sheets[0]['numRows']; $i++) 
	{
		...
		$gcode_r	= addslashes(trim($data->sheets[0]['cells'][$i][$j++])); // 상품코드
		...
		$isnaver	= addslashes(trim(conv_number($data->sheets[0]['cells'][$i][$j++]))); // 네이버 최저가
		...
		$naver_price_url	= addslashes(trim($data->sheets[0]['cells'][$i][$j++])); // 최저가url
		...
		if($isnaver == '2'){ $isnaver = '0'; }

		unset($value);
		$value['mb_id']			= $mb_id; // 판매자ID
		$value['gcode']			= $gcode; // 상품코드
		$value['gcode_r']			= $gcode_r; // 상품코드
		...
		$value['naver_price_url']	= $naver_price_url; // 관리자메모
		$value['shop_state']				= '0'; // 1월 24 추가 승인상태 0: 승인 1: 승인대기
...
<div class="btn_confirm">
	<a href="<?php echo MS_ADMIN_URL; ?>/goods.php?code=xls_mod" class="btn_large">확인</a>
</div>
