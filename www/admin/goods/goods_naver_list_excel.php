...
$excel->setActiveSheetIndex(0)
	...
	->setCellValue($char++.'1', '네이버가격')
	->setCellValue($char++.'1', '최저가')
    ...
    ->setCellValue($char++.'1', '마일리지적립(원)')
    ->setCellValue($char++.'1', '마일리지적립(%)')
    ->setCellValue($char++.'1', '마일리지 개별 적용')
	->setCellValue($char++.'1', '쇼핑포인트적립(P)')
	->setCellValue($char++.'1', '쇼핑포인트적립(%)')
    ->setCellValue($char++.'1', '쇼핑포인트 개별 적용')
    ->setCellValue($char++.'1', '쇼핑포인트결제 허용')
    ->setCellValue($char++.'1', '쇼핑포인트결제(P)')
    ->setCellValue($char++.'1', '쇼핑포인트결제(%)')
...
for($i=2; $row=sql_fetch_array($result); $i++)
{
	...
	$naver_pr = '';
	$row3 = sql_fetch(" select * from naver_list where gcode='{$row['gcode']}' order by price asc limit 1");
	if($row3['price'] < ($row['goods_price'] + $row['sc_amt'])){
		$naver_pr = "N";
	}else{
		$naver_pr = "Y";
	}
	$t_goods_price = '';
	$t_goods_price = $row['goods_price'] + $row['sc_amt'];

	$A_goods_price = '';
	$A_goods_price = $row['supply_price'] + $row['sc_amt'];
	...
	->setCellValueExplicit($char++.$i, $A_goods_price, PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['normal_price'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $t_goods_price, PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row3['price'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $naver_pr, PHPExcel_Cell_DataType::TYPE_STRING)
	...
	->setCellValueExplicit($char++.$i, $row['goods_kv'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['goods_kv_per'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['goods_kv_basic'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['gpoint'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['gpoint_per'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
	->setCellValueExplicit($char++.$i, $row['gpoint_basic'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
    ->setCellValueExplicit($char++.$i, $row['point_pay_allow'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
    ->setCellValueExplicit($char++.$i, $row['point_pay_point'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
    ->setCellValueExplicit($char++.$i, $row['point_pay_per'], PHPExcel_Cell_DataType::TYPE_NUMERIC)
...
header('Content-Disposition: attachment; filename="'.iconv('utf-8','euc-kr','상품').'-'.date("ymd", time()).'.xlsx"');
