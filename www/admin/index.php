...
<div id="main_wrap">
	<section>
		<h2>전체 주문통계<a href="<?php echo MS_ADMIN_URL; ?>/order.php?code=list" class="btn_small">주문내역 바로가기</a></h2>
		<div class="order_vbx">
			<dl class="od_bx1">
				<dt>전체 주문현황</dt>
				<dd>
	...
	<section class="sidx_head01">
		<h2>최근 주문내역<a href="<?php echo MS_ADMIN_URL; ?>/order.php?code=list" class="btn_small">주문내역 바로가기</a></h2>
		<table>
		<thead>
		<tr>
	...
	<section class="sidx_head01">
		<h2>최근 회원가입<a href="<?php echo MS_ADMIN_URL; ?>/member.php?code=list" class="btn_small">회원관리 바로가기</a></h2>
		<table>
		<thead>
		<tr>
			<th scope="col">이름</th>
		...
		<tbody>
		<?php
		$sql = "select * from shop_member where id <> '".encrypted_admin()."' order by index_no desc limit 5";
		$result = sql_query($sql);
		for($i=0; $row=sql_fetch_array($result); $i++){
		?>
		<tr class="tr_alignc">
